 ...
 * @author lizhongwen
 * @since jdk1.6
 * @version 2015年12月31日 lizhongwen
 * @param <T> 模拟写入器
 */
public class MockWriter<T> extends FragmentWriter<T> {
     ...
     */
    @Override
    protected void internalWrite(final DocxExportConfiguration config, final XWPFDocument docx, final T element,
        final String uri) {
        getLogger().error(MessageFormat.format("位置：{0}，出现未知的配置类型：{1}", uri, element.getClass()));
    }
}
