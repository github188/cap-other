/******************************************************************************
 * Copyright (C) 2015 ShenZhen ComTop Information Technology Co.,Ltd
 * All Rights Reserved.
 * 本软件为深圳康拓普开发研制。未经本公司正式书面同意，其他任何个人、团体不得使用、
 * 复制、修改或发布本软件.
 *****************************************************************************/

package com.comtop.cap.document.word.write.writer;

import static com.comtop.cap.document.word.write.DocxConstants.STYLE_TABLE_TITLE;

import java.util.Collection;
import java.util.List;
import java.util.Map;

import org.apache.commons.lang.StringUtils;
import org.apache.poi.xwpf.usermodel.XWPFDocument;
import org.apache.poi.xwpf.usermodel.XWPFTable;

import com.comtop.cap.document.util.ObjectUtils;
import com.comtop.cap.document.word.docconfig.model.DCTable;
import com.comtop.cap.document.word.docmodel.data.CellExtra;
import com.comtop.cap.document.word.docmodel.data.RowExtra;
import com.comtop.cap.document.word.docmodel.data.TableExtra;
import com.comtop.cap.document.word.docmodel.style.TableMargin;
import com.comtop.cap.document.word.expression.ExpressionExecuteHelper;
import com.comtop.cap.document.word.write.DocxExportConfiguration;
import com.comtop.cap.document.word.write.DocxHelper;

/**
 * 列扩展表格写入
 *
 * @author lizhongwen
 * @since jdk1.6
 * @version 2015年11月23日 lizhongwen
 */
public class ExpandColumnTableWriter extends DynamicTableWriter {
    
    /**
     * 根据文档配置写入列动态表格文档片段
     *
     * @param config 文档导出配置
     * @param docx 文档对象
     * @param tableConfig 动态表格配置元素
     * @param uri 文档uri
     * @see com.comtop.cap.document.word.write.writer.DynamicTableWriter#internalWrite(com.comtop.cap.document.word.write.DocxExportConfiguration,
     *      org.apache.poi.xwpf.usermodel.XWPFDocument, com.comtop.cap.document.word.docconfig.model.DCTable,
     *      java.lang.String)
     */
    @Override
    public void internalWrite(final DocxExportConfiguration config, final XWPFDocument docx, final DCTable tableConfig,
        final String uri) {
        ExpressionExecuteHelper executer = config.getExecuter();
        executer.notifyStart();
        String dataSource = tableConfig.getMappingTo();
        Collection<?> datas = null;
        if (StringUtils.isNotBlank(dataSource)) {
            Object result = executer.read(dataSource);
            if (result instanceof Collection) {
                datas = (Collection<?>) result;
            }
        }
        TableExtra tableExtra = this.pretreatment(tableConfig, executer);
        int expandRowNum = 1;
        if (datas != null && !datas.isEmpty()) {
            expandRowNum = ((Collection<?>) datas).size();
        }
        int rowCount = calcTableRowCount(tableExtra);
        int rowNum = rowCount + expandRowNum;
        int colNum = calcTableColumnCount(tableExtra);
        DocxHelper helper = DocxHelper.getInstance();
        try {
            this.columnOverflow(colNum, uri);
        } catch (Exception e) {
            executer.notifyEnd();
            this.getLogger().error(e.getMessage(), e);
            return;
        }
        XWPFTable table = helper.createTable(docx, rowNum, colNum);
        this.createTableHeader(tableExtra, table, rowNum, executer);
        if (datas != null && !datas.isEmpty()) {
            this.fillTableDatas(config, tableExtra, table, uri, expandRowNum);
        }
        this.autoMargin(table, tableExtra.getRows()[rowCount - 1].getCells(), rowCount);
        this.createTableCaption(docx, tableConfig.getName());
        executer.notifyEnd();
    }
    
    /**
     * 创建表头
     *
     * @param config 表格配置
     * @param table 表格对象
     * @param colNum 最大列数
     * @param executer 表达式执行器
     */
    protected void createTableHeader(final TableExtra config, final XWPFTable table, final int colNum,
        final ExpressionExecuteHelper executer) {
        DocxHelper helper = DocxHelper.getInstance();
        int rowCount = calcTableRowCount(config);
        RowExtra[] rows = config.getRows();
        List<TableMargin> margins = config.getMargins();
        for (int rowIndex = 0; rowIndex < rows.length; rowIndex++) {
            RowExtra row = rows[rowIndex];
            helper.setRowStyle(table, rowIndex, STYLE_TABLE_TITLE).setRowBackground(table, rowIndex, "F3F3F3");
            CellExtra[] cells = row.getCells();
            int colIndex = 0;
            for (CellExtra cell : cells) {
                float width = cell.getWidth();
                if (width > 0) {
                    helper.setColWidth(table, colIndex, width);
                }
                if (cell.isMerged()) {
                    colIndex++;
                    continue;
                }
                if (cell.isExtra()) {
                    colIndex = handleExtCell(rowIndex, colIndex, cell, table, executer);
                } else {
                    if (cell.getOccupy() > 1) {
                        int toCell = colIndex + cell.getOccupy() - 1;
                        helper.mergeCellsHorizontal(table, rowIndex, colIndex, toCell);
                    }
                    String text = cell.getHeader();
                    helper.setCellText(table, rowIndex, colIndex, text);
                    if (cell.getColspan() == 1 && cell.getOccupy() > 1) {
                        colIndex += cell.getOccupy();
                    } else {
                        colIndex++;
                    }
                }
            }
        }
        this.marginTable(table, margins);
        if (margins.isEmpty()) {
            helper.setTableHeader(table, rowCount);
        }
    }
    
    /**
     * 处理扩展列表头
     *
     * @param rowIndex 行索引
     * @param startColIndex 起始列索引
     * @param cell 单元格配置
     * @param table 表格对象
     * @param executer 表达式执行器
     * @return 下一个单元格索引
     */
    private int handleExtCell(final int rowIndex, final int startColIndex, final CellExtra cell, final XWPFTable table,
        final ExpressionExecuteHelper executer) {
        DocxHelper helper = DocxHelper.getInstance();
        String veriable = cell.getVeriable();
        List<?> datas = cell.getData();
        int[] slots = cell.getSlots();
        Object[] keys = cell.getKeys();
        String label = cell.getLabel();
        String keyExpr = cell.getKeyExpr();
        int colIndex = startColIndex;
        if (StringUtils.isNotBlank(veriable) && datas != null) {
            executer.write(veriable, datas);
            for (int m = 0; m < slots.length; m++) {//
                int slot = slots[m];
                executer.notifyStart();
                if (StringUtils.isNotBlank(label)) {
                    String txt = ObjectUtils.objectToString(executer.read(label));
                    helper.setCellText(table, rowIndex, colIndex, txt);
                } else {
                    helper.setCellText(table, rowIndex, colIndex, cell.getHeader());
                }
                if (StringUtils.isNotBlank(keyExpr)) {
                    keys[m] = executer.read(keyExpr);
                }
                if (slot > 1) {
                    int toCell = colIndex + slot - 1;
                    helper.mergeCellsHorizontal(table, rowIndex, colIndex, toCell);
                }
                colIndex = colIndex + slot;
                executer.notifyEnd();
            }
        } else {
            colIndex = colIndex + cell.getOccupy();
        }
        return colIndex;
    }
    
    /**
     * 填充表格数据
     * 
     * @param configuration 文档导出配置
     * @param config 表格配置
     * @param table 表格对象
     * @param uri 文档uri
     * @param dataSize 数据量
     */
    protected void fillTableDatas(final DocxExportConfiguration configuration, final TableExtra config,
        final XWPFTable table, final String uri, final int dataSize) {
        ExpressionExecuteHelper executer = configuration.getExecuter();
        DocxHelper helper = DocxHelper.getInstance();
        int rowCount = calcTableRowCount(config);
        int lastHeaderIndex = rowCount - 1;
        RowExtra[] rows = config.getRows();
        for (int i = 0; i < dataSize; i++) {
            RowExtra row = rows[lastHeaderIndex];
            executer.notifyStart();
            CellExtra[] cells = row.getCells();
            int rowIndex = i + rowCount;
            int colIndex = 0;
            for (CellExtra cell : cells) {
                if (cell.getOccupy() > 1) {
                    int toCell = colIndex + cell.getOccupy() - 1;
                    helper.mergeCellsHorizontal(table, i, colIndex, toCell);
                }
                String expression = cell.getDataSource();
                if (StringUtils.isNotBlank(expression)) {
                    Object result = executer.read(expression);
                    if (cell.isExtra()) {
                        Object[] dataKeys = cell.getKeys();
                        if (dataKeys != null && result != null && result instanceof Map) {
                            for (Object dataKey : dataKeys) {
                                Map<?, ?> map = (Map<?, ?>) result;
                                String text = ObjectUtils.objectToString(map.get(dataKey));
                                this.writeCellValue(configuration, table, rowIndex, colIndex, text, uri);
                                colIndex++;
                            }
                        } else {
                            colIndex = colIndex + cell.getOccupy();
                        }
                    } else {
                        String text = ObjectUtils.objectToString(result);
                        this.writeCellValue(configuration, table, rowIndex, colIndex, text, uri);
                        colIndex = colIndex + cell.getOccupy();
                    }
                } else {
                    colIndex = colIndex + cell.getOccupy();
                }
            }
            executer.notifyEnd();
        }
    }
}
