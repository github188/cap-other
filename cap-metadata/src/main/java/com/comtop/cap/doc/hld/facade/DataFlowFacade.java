...
import com.comtop.cap.doc.hld.model.DataFlowDTO;
 ...
 */
@DocumentService(name = "DataFlow", dataType = DataFlowDTO.class)
public class DataFlowFacade extends AbstractExportFacade<DataFlowDTO> {
    ...
    @Override
    public List<DataFlowDTO> loadData(DataFlowDTO condition) {
        String packageId = condition.getPackageId();
        List<PackageDTO> apps = getAllAppPackage(packageId);
        if (apps == null || apps.size() == 0) {
            return null;
        }
        List<DataFlowDTO> alRet = new ArrayList<DataFlowDTO>();
        for (PackageDTO packageDTO : apps) {
            try {
                ...
                for (EntityVO vo : lstEntitys) {
                    DataFlowDTO distributionDTO = new DataFlowDTO();
