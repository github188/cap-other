 ...
 */
public final class MetadataMySQLDataBaseDAO implements IMetadataDataBaseDAO {
    
    /** 日志 */
    private static final Logger LOGGER = LoggerFactory.getLogger(MetadataOracleDataBaseDAO.class);
    
    /** MySQL元数据DAO实例 */
    private static MetadataMySQLDataBaseDAO instance;
     ...
     * 构造函数
     */
    public MetadataMySQLDataBaseDAO() {
     ...
     * @return 转换器实例
     */
    public static synchronized MetadataMySQLDataBaseDAO getInstance() {
        if (instance == null) {
            instance = new MetadataMySQLDataBaseDAO();
        }
        return instance;
    }
    ...
    public MetadataVersionVO getMetadataVersionVOByNodeId(final String nodeId) {
        ...
        try {
            ...
            if (objResultSet != null && objResultSet.next()) {
                ...
                objMetadataVersionVO.setLocked(objResultSet.getInt("objResultSet") == 1 ? true : false);
