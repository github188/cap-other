...
;(function($, C){
    ...
    C.Number.money = function (str, length, other) {
        ...
        str = (Math.round(str * divider) / divider).toString().split(".");
        var str_0 = str[0],
        ...
        //如果设置长度为null，则不对小数进行限制
        if(length !== null){
            return other.prefix + (negative ? '-' : '') + ret.join(other.separator) + (length > 0 ? "." : '') + str_1;
        }else{
            return other.prefix + (negative ? '-' : '') + ret.join(other.separator) + (freeDom.length > 0 ? "." : '') + freeDom;
        }
    };
