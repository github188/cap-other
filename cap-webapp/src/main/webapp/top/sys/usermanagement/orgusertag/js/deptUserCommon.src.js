...
function getRootId(vRootId,vOrgStructureId){
	...
	/**
	 * 是否显示返回上级组织按钮
	 * @return 并返回默认组织对应的组织结构id
	 * */
	function showHigherUpOrNot(){
		var showOrNot = false;
		//如果指定的defaultOrgId属性，则进行显示控制
		if(defaultOrgId){
			//组织结构Id
	...
	*/
	...
	function putToSelectedBox(obj){ 
		...
		//ID替换十六进制中文尖括号《》为原来的应为<>，否认显示报错20160427
...
//			$("#searchDiv").slideUp("fast");
	 ...
	 * @param type 标识是组织标签查询还是用户标签查询
	 * @param operatorType 标识是新查询，还是点击更多数据
	 * @param data 查询返回的结果
	 * **/
