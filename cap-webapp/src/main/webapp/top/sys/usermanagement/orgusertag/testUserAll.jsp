...
<script type="text/javascript">
	var allOrgProperties = "width;height;readonly;isAllowOther;orgStructureId;rootId;userType;isSearch;defaultOrgId;formName;idName;valueName;opts";
	...
	function displayUserTag(){
		...
		cui("#myUserChoose").chooseUser(
			options
		);
		//添加校验
		window.validater = cui().validate();
		window.validater.add('tagUserTest', 'required', { 
			m:'不能为空'
			});
		cui.tip(
		        '#tagUserTest', //要显示Tip的位置，这里指定组件的某个HTML标签，格式是一个jquery选择器表达式
		        {
		            tipEl: $('#tagUserTest')  
		        }
		    );
		...
		//显示按钮
		$("#setUserTagReadonly").css("display","");
		$("#cancelUserTagReadonly").css("display","");
		$("#addValidate").css("display","");
		$("#validateExec").css("display","");
	}
	...
	//设置标签只读
	function setUserTagReadonly(){
		   cui('#tagUserTest').setReadonly(true);  
	}
	
	//取消标签只读
	function cancelUserTagReadonly(){
		   cui('#tagUserTest').setReadonly(false); 
	}
...
<body>
	...
	<input type="button" value="刷新数据" title="清空测试过的数据，重新进行测试" onclick="refreshData();" />   测试函数 chooseCallback(obj)
	<table class="table_list" border="1" id="propertiesTable">
	</table>
	<input value="生成人员标签" type ="button" onclick="displayUserTag();" />
	<input id="setUserTagReadonly"  style="display: none;" value="设置标签只读" type ="button" onclick="setUserTagReadonly();" />
	<input id="cancelUserTagReadonly"  style="display: none;" value="取消标签只读" type ="button" onclick="cancelUserTagReadonly();" />
