...
<div class="top_header_wrap">
   <div class="thw_title" style="font-size:15px;">组织扩展属性编辑</div>
...
<div class="main">
    <table class="form_table" id="attributeTable">
       ...
       <tr>         
		...
		<td class="td_content">
			<span uitype="input" id="attriName" name="attriName" databind="data.attriName" maxlength="20" width="200" 
		 		validate="[{'type':'required', 'rule':{'m': '属性名称不能为空。'}},{'type':'custom','rule':{'against':'isContainSpecialChar','m':'属性名称只能为中英文、数字以及下划线。'}},{'type':'custom','rule':{'against':'isExistRename','m':'已存在该属性。'}}]"></span>
		</td>
	 ...
	 <tr>  
		...
		<td class="td_content">
			<span uitype="input" id="attriCode" name="attriCode" databind="data.attriCode" maxlength="20" width="200" validate="[{'type':'required', 'rule':{'m': '属性编码不能为空。'}},{'type':'custom','rule':{'against':'isCodeContainSpecialChar','m':'属性编码只能为英文、数字以及下划线。'}},{'type':'custom','rule':{'against':'isAttributeCodeUnique','m':'属性编码已存在。'}}]"></span>
		</td>
		<div uitype="input" id="attriField" name="attriField" databind="data.attriField" style="display: none;"></div>
	 </tr>
	 <tr>         
		<td class="td_label" width="30%">
...
function initPage(){
	if(defineId){
		...
		ExtendAttrAction.getExpandDefine(defineId,function(expandDefineData){
		    //填充数据
		    cui(data).databind().setValue(expandDefineData);
		...
		dwr.TOPEngine.setAsync(false);
		ExtendAttrAction.getExpandField(orgStrucId,mark,function(resultData){
		   newAttriField = resultData;
		});
		dwr.TOPEngine.setAsync(true);
		cui(data).databind().set('attriField',newAttriField);
		cui(data).databind().set('sortNo',totalSize);
	}
}
...
function addValue(){
	...
	if(thisRowIdex!=totalRowsLength){
		oTd.innerHTML ='<span  uitype="Input" id="value_'+thisRowIdex +'" maxlength="20" width="200" validate="属性值Value不能为空。"></span><img src="${pageScope.cuiWebRoot}/top/sys/images/add.png" onclick="addValue();" style="cursor:pointer" title="添加属性值"><img src="${pageScope.cuiWebRoot}/top/sys/images/remove.png" onclick="deleteValue('+thisRowIdex+');" style="cursor:pointer" title="删除属性值">';
	}else{
		oTd.innerHTML ='<span  uitype="Input" id="value_'+thisRowIdex +'" maxlength="20" width="200" validate="属性值Value不能为空。"></span><img src="${pageScope.cuiWebRoot}/top/sys/images/remove.png" onclick="deleteValue('+thisRowIdex+');" style="cursor:pointer" title="删除属性值">';
	}
...
function getAttrDefaultValue(){
	...
	for(var i=1;i<=selectedRows.length;i++){
		...
		attrDefaultValue += defaultKey+"-"+defaultValue+';'
	}
            ...
            for (var i = 0; i < inValid.length; i++) {
         	...
         	if(defineId){//编辑
		         	if(oldAttriName!=vo.attriName){
		         		dwr.TOPEngine.setAsync(false);
						var message = "所有组织的<font color='red'>&quot;"+oldAttriName+"&quot;</font>属性都变为<font color='red'>&quot;"+vo.attriName+"&quot;</font>属性，所有值都保留，是否确认修改？";
						cui.confirm(message,{
							onYes:function(){
								ExtendAttrAction.updateExpandDefine(vo,function(){
					                //刷新列表
				            	    window.parent.editCallBack(vo.defineId);
				            	    window.parent.cui.message('扩展属性修改成功。');
				            	    closeSelf();
						    	});
						  	}
						});
         		...
         		ExtendAttrAction.queryExtendDefineList(orgStrucId,2,function(resultData){
			    	if(resultData.count>=20){
			    		cui.alert('最多只能新增20个扩展属性。');
			        	 ...
			        	 ExtendAttrAction.insertExpandDefine(vo,function(id){
				            ...
				            window.parent.cui.message('扩展属性新增成功。');
...
function isContainSpecialChar(){
	var attriName = cui('#attriName').getValue()
...
function isExistRename(){
	var newName = cui('#attriName').getValue()
	var flag = true;
	var objItem={'defineId':defineId,'mark':2,'attriName':newName,'attributeClassify':orgStrucId};
	dwr.TOPEngine.setAsync(false);
...
function isAttributeCodeUnique(){
	var attrubuteCode = cui('#attriCode').getValue();
	var obj = {'defineId':defineId,'mark':2,'attributeClassify':orgStrucId,'attriCode':attrubuteCode};
	var flag = true;
	if(attrubuteCode){
		dwr.TOPEngine.setAsync(false);
