...
<body class="body_layout">
	<div class="list_header_wrap" style="padding:10px;">
		<div class="top_float_left">
			&nbsp;<label style="font-size: 13px;">组织结构：</label>
	        <span uitype="radioGroup" name="attributeClassify" id="attributeClassify" on_change="changeOrgStructure" radio_list="initOrgStructure"> 
			</span>
		</div>
   ...
   <script language="javascript">
	   ...
	   window.onload=function(){
		...
		function initData(grid,query){
			curOrgStrucId = cui('#attributeClassify').getValue();
			if(curOrgStrucId){
		...
		function deleteExpand(){
			...
			if(selectRow !=null && selectRow.length>0){
				...
				var message = "所有组织的<font color='red'>&quot;"+names+"&quot;</font>属性值都会被删除，且无法恢复，是否确认删除？";
				cui.confirm(message,{
					onYes:function(){
						dwr.TOPEngine.setAsync(false);
						ExtendAttrAction.delExpandDefine(selectRow,2,function(){
							selectedRowId="";
							cui("#grid").loadData();
							cui.message("扩展属性删除成功。","success");
				        });
						dwr.TOPEngine.setAsync(true);
				  	}
				});
		...
		function editExpandDefine(defineId){
			...
			var height = 345; //600
			var width =  480; // 680;
			var url='${pageScope.cuiWebRoot}/top/sys/usermanagement/organization/OrgExpandDefineEdit.jsp?totalSize='+totalSize+"&orgStrucId="+curOrgStrucId;
