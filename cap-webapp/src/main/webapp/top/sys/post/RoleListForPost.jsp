		...
		<c:choose>
			<c:when test="${param.postRelationType=='1'}">
				<top:verifyAccess pageCode="TopPostAdmin" operateCode="addOrdelRole">
					<span uiType="Button" label="添加角色" on_click="selectRole" id="selectRole"></span>
				    <span uiType="Button" label="删除角色" on_click="deleteRoleFromPost" id="deleteRoleFromPost"></span>
			    </top:verifyAccess>
		    </c:when>
		    <c:otherwise>
...
<script language="javascript">
	    ...
	    var operateType= "<c:out value='${param.operateType}'/>";   //read 不显示操作按钮,工作台使用该参数
	  ...
	  //被选中列的主键，用于新增及编辑时自动选中
		...
		function initData(grid,query){
			    ...
			    //设置查询条件
			    var queryObj = {pageNo:query.pageNo,pageSize:query.pageSize,fastQueryValue:keyword,sortFieldName:sortFieldName,sortType:sortType,postId:postId,postRelationType:postRelationType,creatorId:globalUserId};
