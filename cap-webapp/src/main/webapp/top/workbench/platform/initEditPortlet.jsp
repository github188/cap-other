    ...
    <div class="ct0pMain">
        ...
        <div id="portletMain">
            <div class="add-container">
                <form id="addForm" action="${pageScope.cuiWebRoot}/top/workbench/PortletAction/editPortlet.ac" method="post">
                    <table border="0" width="1000px">
                        <tr style="line-height: 50px">
                            <td align="right" width="60px">微件名称<span class="pt-must">*</span>：</td>
                            <td width="300px">
                                <input type="hidden" id="portletId" name="portlet.portletId"
                                       value="${portlet.portletId}"/>

                                <span uitype="Input" id="portletName" name="portlet.portletName"
                                      value="${portlet.portletName}"
                                      validate="validatePortletName"></span>
                            </td>
                            <td align="right" width="50px">是否公共<span class="pt-must">*</span>：</td>
                            <td>
                                <div id="isCommon" uitype="RadioGroup" name="portlet.isCommon"
                                     value="${portlet.isCommon}"
                        ...
                        <tr style="line-height: 50px">
                            ...
                            <td width="240px"><span uitype="Input" id="portletRightCode"
                                                    name="portlet.portletRightCode" value="${portlet.portletRightCode}"
                        ...
                        <tr style="line-height: 50px">
                            ...
                            <td>
                                <div id="portletPostion" uitype="RadioGroup" name="portlet.portletPostion"
                                     value="${portlet.portletPostion}">
                        ...
                        <tr style="height: 150px">
                            ...
                            <td colspan="3" valign="top">
                                <span uitype="Textarea" id="portletDescribe" relation="describeTxtNum" byte="false" maxlength="200" name="portlet.portletDescribe"
                                      validate="validatePortletDescribe" value="${portlet.portletDescribe}" width="88%" height="140px"></span>
...
require(["json2", "underscore", "cui", "workbench/platform/js/portlet", "autoIframe"], function (json2, _, cui, portletMgr) {
    ...
    comtop.UI.scan();
    /*根据iscommon的值判断是否要禁用输入*/
    var isCommon = cui("#isCommon").getValue();
    if (isCommon === "1") {
        //获取数据源然后赋值
        cui("#portletTag").setDatasource(tempSelectDataSource);
        cui("#portletTag").setValue('${portlet.portletTag}');
        //cui("#portletTag").setReadonly(true);
        cui("#portletRightCode").setReadonly(true);
    } else {
        var tmpAry = _.filter(tempSelectDataSource,function(item){
        	return item.text !== "公共微件";
        });
        //去重
        tmpAry =  _.uniq(tmpAry,true,function(o){
        	return o.text;
        });
...
function checkIfCommon(val) {
    if (val === '1') {
        ...
        cui("#portletRightCode").setValue('${portlet.portletRightCode}');
        cui("#portletTag").setValue('${portlet.portletTag}');
...
$("#addPortlet").click(function () {
    ...
    if (valid[2]) {
        ...
        PortletAction.ifNameExist(portletId, portletName, function (returnVal) {
            nameExist = returnVal;
        });
        ...
        if (cui("#isCommon").getValue() == '0' && $.trim(cui("#portletTag").getText()) === "公共微件") {
            alert('非公共微件的分类不能使公共微件!');
        }


        //构造新增的微件json数据
        var portletJson = {};
        portletJson.portletId = cui("#portletId").val();
        ...
        //异步提交新增
        dwr.TOPEngine.setAsync(false);
        PortletAction.editPortlet(outerJson2.stringify(portletJson), function (data) {
            if (data === true) {
                cui.message('编辑成功', 'success');
                setTimeout(function(){
