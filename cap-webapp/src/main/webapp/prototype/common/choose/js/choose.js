...
;(function($, C){
    ...
    C.UI.Choose = C.UI.Base.extend({
        ...
        _textWidth: function(which) {
            var text = which.val(),
                text = $("<div style='display:none'/>").text(text).html(),
                font = which.css("font");
        ...
        _createDom: function() {
            ...
            if ($.type(this.options.value) === "string") {
        ...
        _aHandler: function(tg) {
            var $tg = $(tg),
			  dialog = cui("#topdialog_" + this.options.id),
			  $iframe = null,
			  msg = {
			    selected : this.options.value.join(';'),
			    dataSource : this.options.dataSource
			  }
			  id = this.options.id;
            if ($tg.attr('flag') == 'del') {
                this._deleteData($tg.parent().attr('title'));
            } else {
                ...
                var url = webPath + "/common/choose/ChoosePage.html?id=" + this.options.id + "&chooseType=" + this.options.uitype + "&chooseMode=" +
                    this.options.chooseMode + "&orgStructureId=" + this.options.orgStructureId + "&dataSource=" + (this.options.dataSource || '') + "&winType=" + this.options.winType,
                    
                //"选择人员":"选择组织"
                    winTitle = this.options.uitype === 'ChooseUser' ? "\u9009\u62e9\u4eba\u5458" : "\u9009\u62e9\u7ec4\u7ec7",
                    winSize = this._getWindowSize();

                if (this.options.winType === "window") {
                    var outerWindow = window.open(url, "ChoosePage", "left=" + winSize.offsetLeft + ",top=" + winSize.offsetTop + ",width=" + winSize.width + ",height=" + winSize.height + ",menu=no,toolbar=no,resizable=no,scrollbars=no");
                   
                  messenger.addTarget(outerWindow, this.options.id);
                  messenger.targets[this.options.id].send(msg);
                } else {
                    if (!dialog.length) {
                        dialog = cui.dialog({
                        ...
                        $iframe = $("#topdialog_" + id + " iframe");
                        messenger.addTarget($iframe[0].contentWindow, this.options.id);
                        $iframe.load(function(){
                          messenger.targets[id].send( JSON.stringify(msg) );
                        });
                    }else{
                        messenger.targets[id].send(JSON.stringify(msg));
                    }
        ...
        _buildSelect: function() {
            var html = [],
                len = this.options.value.length;
            //如果是文本模式
            if (this.options.textmode) {
                var showName = "";
                for (var i = 0; i < len; i++) {
                    showName = this.options.value[i];
                    //显示时用showName
                    html.push(showName);
                }
                return html.join(";");
                ...
                var showName = "";
                for (var i = 0; i < len; i++) {
                    blockStyle = "block_cross";
                    showName = this.options.value[i];
                    html.push('<div class="' + blockStyle + ' ' + multiStyle + '" title="' + showName + '"><span class="c_content">' + showName + '</span><a flag="del" href="#" class="block_delete"></a></div>');
                }
                return html.join("");
            }
        },
...
var messenger;
$(function(){
    messenger = new Messenger('parent', 'chooser');             
	messenger.listen(function (msg) {
		var action = {
		  "winClose" : function(obj){
		     cui("#topdialog_" + obj.id).hide();
		  },
		  "submit" : function(obj){
			  cui("#" + obj.id).__setValue(obj.value && obj.value.split(';'));
		  }
		},
		obj = JSON.parse(msg);
