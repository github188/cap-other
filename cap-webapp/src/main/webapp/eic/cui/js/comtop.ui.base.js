...
comtop.version = 'V4.1.0-beta3-Bulid20131203-02';
//=============================???¡Â???================================
;(function(C){
    /**
     * ?????//TODO ????????console?????????????????
    ...
    /**
     * ??????????
     * @param {String} name  ??'comtop.ui.button'
     * @return {Object}
     */
...
// Back Compat <1.8 extension point
    ...
    jQuery.fn.offset = function( options ) {
        ...
        if(compat == 'BackCompat' && !!window.ActiveXObject){
            return {
                top: box.top + Math.max(window.document.documentElement.scrollTop, window.document.body.scrollTop) - clientTop,
                left: box.left + Math.max(window.document.documentElement.scrollLeft, window.document.body.scrollLeft) - clientLeft
            }
        }
                ...
                if ( val === undefined ) {
                    ...
                    if(compat == 'BackCompat' && !!window.ActiveXObject){
                        ...
                        return win ? (prop in win) ? win [ prop ] :
                            Math.max(window.document.documentElement[ method ], window.document.body[ method ]) :
                            elem[ method ];
                    }
...
;(function($, C){
    var $html = $('html');
    $html.attr("style", "font-family:isIE\\0; border:none; *font-family:isIE7; " +
    ...
    var font_family = $html.css("font-family");
    if( /isIE(?!\\|0)/.test(font_family) ) {
        C.Browser[font_family] = true;
        C.Browser["notIE"] = false;
        if(font_family === "isIE" || font_family === "isIE6" || font_family === "isIE7") {
            C.Browser["isIE"] = true;
        }
        else if( $html.css("border") === undefined) {
...
//================================JCT=====================================
/**
 * JavaScript Common Templates(jCT) 3(??3??)
...
;(function(C){
    ...
    jCT.prototype={
        ...
        ExecChilds:function(childs,exec){//???childs???????§Ô???????????????????Exec????
        ...
        BuildChilds:function(childs){//??????jCT????,?????RunNow
    ...
    jCT.Instance.prototype={
        ...
        Parse:function(self){
            ...
            while (this.Slice(tag.block,p[4],p,max)){//????2??
                p1=[0,0,0,0,p[0]];
        ...
        Slice:function(tn,b1,p,max){//??string??2?¦Ç??2??
            var begin=tn.begin;
            var end=tn.end;
            var e1,b2,e2;
...
;(function($, C) {
    ...
    window.cui = function(el){
        ...
        if(!cuiObj.cuiCreated) {
            cuiObj.cuiCreated = function(method, arg) {
                if( this[method] ) {
                    this[method].apply(this, Array.prototype.slice.call(arguments, 1) );
                } else {
                    var cuiCreated = this.data("cuiCreated") || [];
                    cuiCreated.push({
                        "method" : method,
                        "arg"    : Array.prototype.slice.call(arguments, 1)
                    });
                    this.data("cuiCreated", cuiCreated);
                }
                delete this.cuiCreated;
                return this;
            }
        }
        return cuiObj;
    ...
    function registerCmp (clazz, superClass) {
        ...
        if ($.fn[uncapliseUIType] == null) {
            $.fn[uncapliseUIType] = function (options){
                var $el = $(this).eq(0);
                var scope = C.UI[uitype].prototype.options.scope;

                //???selector???????????selector
                $el.selector = $el.selector.replace('.slice(0,1)', '');
                if(!$el.selector && scope != 'uitype'){
                ...
                var cuiObj = null;
                if (scope == null) {
                    cuiObj = $el.data('uitype'); //8mm/100;
                } else if (scope == 'uitype') {
                    cuiObj = $el.data(uitype); //8mm/100;
                }
...
;(function($, C){
    ...
    C.UI.templatePath = null;   //??????¡¤??
    C.UI.componentList = [];    //????????UI???
    ...
    C.UI.Base = C.Class.extend({
        ...
        /**
         * ???????????
         * @param {jQuery | HTMLElement} target ???????¦Ë??
         * @param {String} tpl ????????
         * @param {Object} options ??????
         * @param {Boolean} isAppend ??????????????DOM
         * @return {JQuery}
         * @protected
         */
        ...
        /**
         * ????targetCmp???
         * @param {Object} cmpOpts ????targetCmp????options
         * @private
         */
        ...
        __init:function(options){
            ...
            for (tpl in self.tpls) {
                if (self.autoTpls[tpl] == true){
                    ...
                    if (/[DIV|SPAN]/.test(tagName) && $.trim($target.html()) == '') {
            ...
            if(typeof view === 'string'){
                ...
                for(var i = 0; i < bindList.length; i ++){
                    ...
                    for(var k = 0, len = eventList.length; k < len; k ++){
                        ...
                        //???a???????javascript:;????????return????????onbeforeunload???
                        eventString.push(
                            'on', singleEvent[0], '="',
        ...
        /**
         * ??????????
         * @param {..|jQuery|HTMLElement} $el ???????????????????options.el??????????
         * @protected
         */
        ...
        /**
         * ????????
         * @param {Boolean} flag ?????????true??????false????????????false
         * @public
         */
         ...
         * ???????
         * @param {String} str  ???
         * @param {String} type ??????????????? "boolean" "number" "string" "json" "function"
         * @param {Boolean | Number | String | Function | Object} defaultValue ????
    ...
    C.proxyHandler = function(e, selector, eventEl, func){
        ...
        //???target??A?????????click??????????IE???????????href="javascript:;"?????????onbeforeunload????
    ...
    C.UI.scanTemplateUrl = function(tmplName){
        ...
        $OScript = $('script[src*="cui/js/comtop.ui"]');
        ...
        scriptOP.push({path: tmpPath, attrTmpl: ''});
    ...
    function _scanHTML(el, name){
        var html = el.outerHTML.match(/<[^>]*>/)[0].replace(/[\r\n\t]/g,'');
    ...
    $(window).bind('unload', function(){
        if(comtop.Browser.isIE && window.CollectGarbage){
            window.CollectGarbage();
        }
        window.comtop = undefined;
        window.cui = undefined;
    });
})(window.comtop.cQuery , window.comtop);
 ...
 */
;(function($, C){
    //==========by ke shang fu(start)
    C.Dom.minWidth = function($el, minWidth, padding, border) {
        if(C.Browser.isIE){
            if($el.width() < minWidth){
                if(C.Browser.isQM){ //IE???????????padding??border
                    padding = padding || 0;
                    border = border || 0;
                    $el.width(minWidth + padding + border);
                } else {
                    $el.width(minWidth);
                }
            }
        } else {
            $el.css({minWidth: minWidth});
        }

    };

    C.Dom.maxWidth = function($el, maxWidth, padding, border) {
        if(C.Browser.isIE){
            if($el.width() > maxWidth){
                if(C.Browser.isQM){ //IE???????????padding??border
                    padding = padding || 0;
                    border = border || 0;
                    $el.width(maxWidth + padding + border);
                } else {
                    $el.width(maxWidth);
                }
            }
        } else {
            $el.css({maxWidth: maxWidth});
        }

    };

    C.Dom.minHeight = function($el, minHeight, padding, border) {
        if(C.Browser.isIE){
            if($el.height() < minHeight){
                if(C.Browser.isQM){ //IE???????????padding??border
                    padding = padding || 0;
                    border = border || 0;
                    $el.height(minHeight + padding + border);
                } else {
                    $el.height(minHeight);
                }
            }
    ...
    C.Dom.maxHeight = function($el, maxHeight, padding, border) {
        if(C.Browser.isIE){ //IE???????????padding??border
            if($el.height() > maxHeight){

                if(C.Browser.isQM){
                    padding = padding || 0;
                    border = border || 0;
                    $el.height(maxHeight + padding + border);
                } else {
                    $el.height(maxHeight);
                }
            }
     ...
     * ??????????????
     */
    C.Dom.DocumentSize = function(doc) {
        ...
        if( ! C.Browser.isIE ) {
            ...
            myWidth = $(doc).width();
            myHeight = $(doc).height();
        }else if( doc.documentElement && ( doc.documentElement.scrollWidth || doc.documentElement.scrollHeight ) ) {
            //IE 6+ in 'standards compliant mode'

            var scrollHeight = doc.documentElement.scrollHeight;
            var offsetHeight = doc.documentElement.offsetHeight;
            var scrollWidth = doc.documentElement.scrollWidth;
            var offsetWidth = doc.documentElement.offsetWidth;

            myWidth = Math.max(scrollWidth, offsetWidth);
            myHeight = Math.max(scrollHeight, offsetHeight);

        } else if( doc.body && ( doc.body.scrollHeight || doc.body.scrollWidth ) ) {
            //IE 4 compatible

            var scrollHeight = doc.body.scrollHeight;
            var offsetHeight = doc.body.offsetHeight;
            var scrollWidth = doc.body.scrollWidth;
            var offsetWidth = doc.body.offsetWidth;
    ...
    C.MQ = {
        ...
        sub: function(topic, handler) { //???????
            ...
            this.topicList[topic].push(handler);

        },

        unsub: function(topic, handler) { //????????
            var handlerList = this.topicList[topic];
            if(!handlerList){
                return;
            }
        ...
        pub: function(topic, data) { //???????
            ...
            if(!handlerList || handlerList.length==0){
                console.warn('no widget has subscribe this topic ' + topic);
                return;
            }
     ...
     * @return {String} ???
     */
    C.Date.format = function(source, pattern){
        ...
        var o = {
            ...
            "w+" :  C.Date.getOnWeek(source).week
        };
     ...
     * @return {Date} ???????
     */
    C.Date.parse = function(str, format){
        str = $.trim( str );
        if( str === '' ){
            return null;
        }
        ...
        if( validDate(str,format) ){
            ...
            //???format?????????????????????¡¤?????????¡¤?????????????????
            //?³2012???3??????????new Date(2012, 8, 1, 0, 0, 0)
            if(index[6] >= 0 && index[1] < 0){
                ...
                var range = C.Date.getWeekRange(arDate[0], arDate[7]);
     ...
     * @returns {string} ???
     */
    C.Number.money = function (str, length) {
        ...
        str = parseFloat(str, 10) || 0;
        str = (Math.floor(str * divider) / divider).toString().split(".");
        var str_0 = str[0],
        //str_1 = (str[1] || "" + "00").slice(0, 2),
            str_1 = (str[1] || "" + cents).slice(0, length),
            len   = str_0.length,
            start = len % 3,
            ret   = start ? [str_0.slice(0, start)] : [];
        for (var i = 0, size = Math.floor(len / 3); i < size; i += 1) {
            ret.push(str_0.slice(start + i * 3, start + i * 3 + 3));
        }
        if(str_1.length < length){
            for(var j = 0, len = str_1.length; j < length - len; j ++){
                str_1 += '0';
            }
        }
        return (negative ? '-' : '') + ret.join(",") + (length > 0 ? "." : '') + str_1;
    };
        ...
        $.ajax({
            ...
            error: function(xhr, textStatus) {
                if (error && error.call) {
                    ...
                    //KSSP-2978: ????????????????(???????DOM???????),???????,????:status==0,??????????.Fuyx,2011-07-08
                } else {
                    cui.error('??????');
                }
            }
