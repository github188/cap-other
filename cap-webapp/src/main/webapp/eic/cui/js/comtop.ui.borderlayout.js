;(function(C){
    ...
    C.UI.Borderlayout = C.UI.Base.extend({
        ...
        _createDom: function () {
            ...
            for (var i = items.length; i--;) {
                ...
                if (position !== "center") {
                    ...
                    move = '<div class="' + (item.resizable ? 'bl_move' : 'bl_border' ) + '" style="' + dir + ':' + item.split_size + 'px;">' + ( (fixed[position] && item.collapsable && item.show_expand_icon) ? '<div class="bl_fold bl_unfold"></div>' : '' ) + '</div>';
                ...
                if (item.is_header) {
                    header = '<div class="bl_header" style="height:' + item.header_height + 'px;line-height:' + item.header_height + 'px;">' + item.header_content + '</div>';
                }
                frag += '<div class="bl_inner" header="' + (item.is_header ? item.header_height : 0) + '">' + header + '<div class="bl_box_' + position + ' bl_box' + this.id + '">' + ( item.url !== "." ? '<iframe url="' + item.url + '" frameborder="0" class="bl_iframe_page"></iframe>' : '' ) + '</div></div>';
        ...
        _setLayout: function (widths, heights) {
            ...
            var layout = this.layout = $.extend(widths, heights);
            for (var v in layout) {
         ...
         * @private
         */
        _setBoxLayout: function (box_css) {
        ...
        _setBoxHeight: function (lay, change) {
            ...
            lay.find(".bl_box" + this.id).each( function (j, ele) {
                ...
                var height = box_height[ position ] = Math.max(box_height[ position ] += change, 0);
                $(ele).css("height", box_height[ position ]);
                if (!height) {
                    $(ele).hide();
                } else {
                    $(ele).show();
