...
(function (C) {
    ...
    C.UI.Gridx = C.UI.Base.extend({
        ...
        _init: function () {
            ...
            this.box_height        = 0;
            this.pagination_height = this.options.pagination ? 40 : 0;
            //html???
        ...
        _backupQuery: function () {
            this.backup_query = $.extend({}, this.getQuery());
        },
        /**
         * ?????????????sort_type????
         * @private
        ...
        __getDomTh: function () {
            ...
            if (len === 0) {
                var new_tr = document.createElement("tr"),
                    thead = this.el.find("thead").eq(0);
                new_tr.innerHTML = thead.html();
                thead.append(tr);
                len = 1;
            }
            if (len === 1) {//???????
                var ths = this.el[0].getElementsByTagName("th"),
                    dom_th = [];
                ...
                for (var i = 0; i < len; i += 1) {//????
                    ...
                    for (var j = 0; j < len_th_i; j += 1) {//????
                        var th_i_j = th_i[j],
                            cell_span = th_i_j.colSpan,
                            row_span = th_i_j.rowSpan;
                        for (var k = 0; k < row_span; k += 1) {//row_span
                            if (!thead_map[i + k]) {
                                thead_map[i + k] = [];
                            }
                            for (var l = 0; l < cell_span; l += 1) {//cell_span
                                thead_map[i + k].push(th_i_j);
                            }
                        }
                    }
                ...
                this.dom_th = $.unique($.extend([], thead_map[len - 1]));
            }
        },
        /**
         * ???????ß÷???????.
        ...
        __getTagsPropertys: function () {
            ...
            for (var i = 0; i < col_size; i += 1) {
                ...
                if (format !== null) {
                    if (format === "money") {
                        col_render[i][1] = {"callback": this.format_fn.money};
                    }
                    if (/(\s|\-|:|yy)/i.test(format)) {
                        col_render[i][1] = {
                            "format": format,
        ...
        __initWidthAndHeight: function () {
            ...
            this.grid_width = ( rewidth || parseInt(opts.gridwidth, 10)) - 2;
            var gridheight  = opts.gridheight;
            if (gridheight === "auto") {
                gridheight       = "500px";
                this.auto_height = true;
            }
            this.grid_height  = ( reheight || parseInt(gridheight, 10) ) - 2;
            //table????
            this.thead_height = this.el.find("tr").length * 31;
            this.box_height   = this.grid_height - this.pagination_height - (opts.titlelock ? this.thead_height : 0);
            this.table_width  = opts.adaptive ? this.grid_width - 17 : parseInt(opts.tablewidth, 10);
        },
        /**
         * ?????????ß÷????
         * @private
        ...
        __createLayoutDom: function () {
            ...
            if (opts.loadtip) { //loading
                html.push("<div class='grid_loading_bg'></div><div class='grid_loading_box'><span>Loading...</span></div>");
            }
        ...
        __renderHead: function () {
            ...
            for (var i = 0; i < col_size; i += 1) {
                ...
                var th_html = [
                    ...
                    '<a class="grid_thead_text">' + thead_text[i] + '</a>'
                ];
                if (colhidden) {
                ...
                th_html.push('</span>');
                if (i < fixcolumnnumber) {
                    th_html.push(thead_text[i]);
                }
                th_html.push('</div>');
                dom_th_i.innerHTML = th_html.join("");
            }

            this.grid_head_table.html(el.find("thead").eq(0).prepend(tr));
            //????fixed dom ????z-index???
            ...
            this.grid_head_table.on("click", function (event) {
                ...
                if (class_name === "grid_all_check") {
                    ...
                    if (typeof selectall_callback === "function") {
                        ...
                        selectall_callback(checked ? self.data : [], checked);
                    }
                    return;
                }
                //????
        ...
        _setOptsSortNameAndSoryType: function (name, type) {
            ...
            if (sortname.length === sortstyle) {
                var del_bind_name = sortname.shift();
                sorttype.shift();
                //???????????????????ß÷????????
                var bind_name     = this.bind_name;
                var dom_th        = this.dom_th;
                for (var j = this.col_size; j--;) {
                    if (del_bind_name === bind_name[j]) {
                        $(dom_th[j]).removeClass("grid_sort_desc").removeClass("grid_sort_asc");
                        break;
                    }
                }
            }
        ...
        __tbodyClickEventBind: function () {
            var opts                 = this.options,
                ...
                timeout              = null,
                target               = null;
            self.grid_tbody.on("click", function (event) {
                event.stopPropagation();
                target = event.target;
                ...
                var on_click = target.getAttribute("onclick") || "";
                var reg_exp = new RegExp(target.tagName, "i");
                if (reg_exp.test("a;button") || reg_exp.test("img") && (clickid !== "" || on_click !== "" )) {
                    if (clickid !== "") {
            ...
            self.grid_tbody.on("dblclick", function (event) {
                event.stopPropagation();
                if (rowdblclick_callback) {
                    clearTimeout(timeout);
                    target       = event.target;
                    ...
                    rowdblclick_callback(self.data[tr_index], tr_index);
                }
            });
            function clickCallback(tr_index) {
                ...
                if (typeof opts.rowclick_callback === "function") {
                    ...
                    opts.rowclick_callback(self.data[tr_index], flag, tr_index);
                }
            }
        },
        /**
        ...
        __tbodyMouseEventBind: function () {
            var self            = this,
                ...
                tr;
            self.grid_tbody.on("mouseover", function (event) {
                var target   = event.target,
                    tag_name = target.tagName;
                if (tag_name === "TR" || tag_name === "TBODY") {
                    return;
                }
                var td           = $(target).closest("td"),
                    td_index     = td[0].cellIndex,
                ...
                if (typeof bind_name_i !== "number" && !td.attr("title")) {
                    if (typeof titlerender === "function") {
                        var title = titlerender(self.data[tr_index], bind_name_i);
                        if (title) {
                            td.attr("title", title);
                        } else {
                            if (!content_box.find("*").length) {
                                td.attr("title", content_box.html());
                            }
                        }
        ...
        _colHidden: function () {
            ...
            for (var i = this.col_size; i--;) {
                if (col_hidden[i] === true) {
                    if (col_width_backup[i] === undefined) {
        ...
        _isScrolling: function (un_set_col_width) {
            ...
            if (this.options.adaptive && !this.is_ittab) {
                ...
                if (box_height < content_height) {
                    ...
                    this.grid_scroll.css("width", grid_width - 17);
                } else {//??ß€?????
                    this.table_width = grid_width;
                    table_width = grid_width + 1;
                    this.grid_scroll.css("width", "");
                }
                ...
                if (grid_width < table_width &&
                    ...
                    grid_scroll.css("width", "").show().children("div").eq(0).css("width", table_width - 18);
                ...
                if (grid_width < table_width + 17 &&
                    ...
                    box_height < content_height + 17) {
                    grid_scroll.css("width", grid_width - 17).show().children("div").eq(0).css("width", table_width);
        ...
        _setAutoHeight: function () {
            if (this.auto_height) {
                var margin_bottom = parseInt(this.grid_table_box.css("marginBottom"), 10) || 0;
                var table_height = this.el.height();
                this.setHeight(table_height + margin_bottom + this.thead_height + (this.options.pagination ? 40 : 0) + 2);
            }
        },
        /**
         * ????:?????????????,col???????.
         * @private
         */
        _setLayout: function (no_scrolling) {
            this._setStyleWidth();
            //??????????
        ...
        _setTheadHeight: function () {
            ...
            for (var i = dom_fixed_0.length; i--;) {
                ...
                var th_height = $(dom_fixed_0_i).parents("th").height() - 1 + "px";
        ...
        setDatasource: function (data, totalSize) {
            ...
            var row_size = this.row_size = data.length;
            ...
            //totalSize?Å£,??????
            if (row_size !== 0 && this.total_size !== 0 && this.total_size !== totalSize) {
                this.total_size = totalSize;
                this._changepages();
            }
            this._isEmpty();
        ...
        _initProperty: function () {
            ...
            this.end_row          = 0;
            this.render_complete  = false;
            this.grid_head.find(".grid_all_check").prop("checked", false);
        ...
        _changepages: function () {
            this.pagination_obj.setCount(this.total_size);
        ...
        _renderTbody: function (add_size, position) {
            ...
            if (!isNaN(add_size)) {
                ...
                if (typeof position === "number" && position < end_row - 1) { //?ßﬁ??????
                    //???index????.
                    var dom_tr = this.dom_tr;
        ...
        _dataToDom: function (data, start, end) {
            ...
            for (var j = start; j < end; j += 1) {
                ...
                if (ispk) {
                    table.push(' pkey="');
                    table.push(data_j[primarykey].toString());
                    table.push('"');
                }
                ...
                for (var i = col_start; i < fixcolumnnumber; i += 1) {
                    ...
                    if (num_col === i) {
                        ...
                        if (!bind_name_i) {
                            ...
                            value = typeof bind_dot_name_i === "undefined" ? data_j[bind_name_i] : data_j[bind_dot_name_i[0]][bind_dot_name_i[1]];
                        }
                    }
                    var tag_html   = "";
                    ...
                    if (col_render_i_0) {
                        ...
                        var method = col_render_i_0.method;
                        switch (render) {
                            case "colrender_fn" :
        ...
        _appendRows: function (start, end) {
            ...
            if (typeof loadcomplate_callback === "function" && end > start) {
                loadcomplate_callback();
            }
        },
        /**
         * ????????????????????
        ...
        setQuery: function (query) {
            ...
            var opts      = this.options;
            opts.pagesize = query.pageSize;
            opts.pageno   = query.pageNo;
            var sortname  = opts.sortname = $.extend([], query.sortName),
                sorttype  = opts.sorttype = $.extend([], query.sortType),
                sortstyle = opts.sortstyle;
            if (sortname.length > sortstyle || sorttype.length > sortstyle) {
                sortname.length = sortname.length = sortstyle;
            }
            this._setPageSize();
            this._setSortTypeObj();
        },
        ...
        selectRowsByIndex: function (index, flag) {
            ...
            if (selectrows === "single") {
                this._selectRows(index[0], flag);
            } else {
                for (var i = 0, len = index.length; i < len; i += 1) {
                    this._selectRows(index[i], flag);
                }
            }
        ...
        selectRowsByPK: function (pks, flag) {
            ...
            return this.selectRowsByIndex(this._pkToIndex(pks), flag);
        },
        /**
         * ????ß”???.
         */
        ...
        removeData: function (rows) {
            if (typeof rows === "number" || $.type(rows) === "array") {
                ...
                if (typeof removedata_callback === "function") {
                    removedata_callback(remove_datas);
                }
            }
        },
        /**
        ...
        changeData: function (new_data, index) {
            ...
            if (typeof pk_value !== "undefined") {
                var new_index = this._pkToIndex(pk_value)[0];
        ...
        getSelectedRowData: function () {
            ...
            var data = this.data;
            if (selectrows === "single") {
                return [data[this.single_checked]];
            }
            if (this.multi_checked_num === this.row_size) {
                return this.data;
            }
        ...
        ittab: {
            _init: function (self) {
                var grid_overlay = self.grid_overlay,
        ...
        hideCol: {
            ...
            _init: function (self) {
                ...
                this.grid_hidecol_list = hide_col.children(".grid_hidecol_list")[0];
            ...
            bindEvent: function (self, obj) {
                var col_size = self.col_size,
                    grid_offset_left = self.grid_box.offset().left,
                ...
                self.grid_head_table.on("click", function (event) {
                    ...
                    if (target.className === "grid_select") {
                        ...
                        var height = self.box_height - self.thead_height - 40;
