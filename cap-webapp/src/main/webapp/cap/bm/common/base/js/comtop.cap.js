...
(function(cap){
	cap.array = {
         ...
         * 
         * @param  {[type]}  arr [数组必须是基础类型数组]
         * @return {Boolean}     [如果重复则返回重复的值,不重复则返回null]
         */
        isRepeat4: function(arr) {
            var hash = {};
            for (var i in arr) {
                if (hash[arr[i]])
                    return arr[i];
...
/**
 * 按区域设置控件为自读状态
 * @param unReadonlyAreaClass 不设置自读区域
 * @param unbindNodeObjAry 不取消节点事件
 * @param disabledNodeObjAry 设置自读状态（针对原生控件）
 * @param addClassNodeAry 添加不设置为自读状态的节点（设置在当前节点的父节点上）
 */
cap.setReadonlyArea = function(unReadonlyAreaClass, unbindNodeObjAry, disabledNodeObjAry, addClassNodeAry){
	comtop.UI.scan.setReadonly(true);
	if(typeof addClassNodeAry != 'undefined' && addClassNodeAry.length > 0){
		$(addClassNodeAry.join(",")).parent().addClass('unReadonlyArea');
	}
	comtop.UI.scan.setReadonly(false, '.'+unReadonlyAreaClass);
	//解除自读控件的所有绑定事件（原生onclick比较特殊，只能删除属性）
	$(unbindNodeObjAry.join(",")).unbind().removeAttr("onclick");
	//针对原生控件设置
	$(disabledNodeObjAry.join(",")).attr("disabled", 'disabled');
	var btnAry = $("span[uitype='button'], span[cui_button]").not($(".unReadonlyArea [uitype='button'], .unReadonlyArea [cui_button]"));
	for(var i=0, len=btnAry.length; i<len; i++){
		cui(btnAry[i]).disable(true);
	}
}