...
<head>
	...
	<top:script src='/cap/dwr/interface/ConfigRepositoryCroListPageAction.js'></top:script>
	<top:verifyRight resourceString="[{menuCode:'cap_cdp_configRepositoryCroListPage'}]"/> 
...
var configRepositoryList=[];

var hideQueryCondition="hide";
var configRepositoryCroEditPage='${pageScope.cuiWebRoot}'+'/cap/bm/cdp/configRepositoryCroEditPage.ac';
var configRepositoryCroViewPage='${pageScope.cuiWebRoot}'+'/cap/bm/cdp/configRepositoryCroEditPage.ac';
...
function gridDatasource(obj, pageQuery) {
	var queryVarName = 'configRepository';
	var query = {};
	if(queryVarName !== ''){
	...
	ConfigRepositoryCroListPageAction.dwrInvoke(dwrInvokeParam,{callback:function(result){
		var returnValueVarName= 'configRepositoryList';
		if(returnValueVarName === ''){
  		...
  		if(saveQueryData=='yes'){
  			cap.setSessionAttribute({'configRepository' : cap.cacheGridAttributes(obj.getQuery(), window['configRepository'])});
  		}
  	},
...
function clearQueryCondition(){
    ...
    cui(configRepository).databind().setEmpty();
    
    //获取查询条件表单所有数据
    var gridId='#uiid-2007403086638078';
    var data = cui(configRepository).databind().getValue();
...
function addBizObject(event, self, mark) { 
	var pageJumpURL=cap.getforwardURL(configRepositoryCroEditPage);
...
function deleteRow(){
	...
	ConfigRepositoryCroListPageAction.queryBuildInfoConfigRepoList(selects,function(data){
	...
	cui.confirm("确定要删除这"+selects.length+"条数据吗？",{
		onYes:function(){
			...
			var aliasEntityId = 'com.comtop.cap.bm.cdp.entity.ConfigRepository';
			aliasEntityId = aliasEntityId != '' ? aliasEntityId :  'com.comtop.cap.bm.cdp.entity.ConfigRepository';	
			var dwrInvokeParam = cap.getDwrInvokeParam(aliasEntityId,'delConfigRepo',paramArray);
			ConfigRepositoryCroListPageAction.dwrInvoke(dwrInvokeParam,{callback:function(result){
				//数据刷新前处理
...
function getDicDataByCode(rd, index, col) {
	...
	if(colDicSet.length == 0){
		var dicData = cap.getDicData('cap_cdp_pkg_format');
		if(dicData.length > 0){
			cap.dicDatas.push({"attrs":[col.bindName],"list":dicData,"code":'cap_cdp_pkg_format'});
		}
		colDicSet = dicData;
	}
...
var uiConfig={
    ...
    "uiid-2007403086638078":{
        ...
        "columns":[{"bindName":"name","name":"配置库名称","render":"a","options":"{'url':configRepositoryCroEditPage,'params':'id','targets':'_self'}"},{"bindName":"pkgFormat","name":"发布包类型","render":"getDicDataByCode"},{"bindName":"repositoryUrl","name":"源码库URL"},{"bindName":"createTime","format":"yyyy-MM-dd","name":"创建时间"}],
        "gridheight":"auto",
