...
<head>
	...
	<top:script src='/cap/dwr/interface/DbconfigCroListPageAction.js'></top:script>
	<top:verifyRight resourceString="[{menuCode:'cap_cdp_dbconfigCroListPage'}]"/> 
...
var dbconfigCroViewPage='${pageScope.cuiWebRoot}'+'/cap/bm/cdp/dbconfigCroEditPage.ac';
...
function gridDatasource(obj, pageQuery) {
	var queryVarName = 'databaseConfig';
	var query = {};
	if(queryVarName !== ''){
	...
	DbconfigCroListPageAction.dwrInvoke(dwrInvokeParam,{callback:function(result){
		var returnValueVarName= 'databaseConfigList';
		if(returnValueVarName === ''){
  		...
  		if(saveQueryData=='yes'){
  			cap.setSessionAttribute({'databaseConfig' : cap.cacheGridAttributes(obj.getQuery(), window['databaseConfig'])});
  		}
  	},
...
function clearQueryCondition(){
    ...
    cui(databaseConfig).databind().setEmpty();
    
    //获取查询条件表单所有数据
    var gridId='#uiid-2007403086638078';
    var data = cui(databaseConfig).databind().getValue();
