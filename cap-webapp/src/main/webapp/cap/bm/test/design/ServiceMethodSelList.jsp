...
<div class="list_header_wrap">
	<div class="top_float_left">
		<div class="thw_title" style="margin-left:11px;margin-top:0px;">
			<font id="pageTittle" class="fontTitle">服务方法列表</font> 
		</div>
	</div>
 ...
 <table uitype="Grid" id="serviceMethodGrid" primarykey="modelId" sorttype="1" datasource="initData" pagination="false"  pagesize_list="[10,20,30]"  
...
<script language="javascript"> 
	...
	var serviceModelId = "${param.serviceModelId}";
	...
	function initData(tableObj,query){
		if(nodeType == "app"){
			...
			ServiceObjectFacade.queryMethodByModelId(serviceModelId, function(data){
				if(!data.list){
				    tableObj.setDatasource([], 0);
				}else{
					tableObj.setDatasource(data.list, data.list.length);
				}
			});
		}
	}
	...
	//点击确定回调传值到父页面
	function enSure() {
		var selectData = cui("#serviceMethodGrid").getSelectedRowData();
		if(selectData == null || selectData.length == 0){
			cui.alert("请选择服务方法。");
			return;
		}
		window.parent.parent.selServiceMethodBack(selectData);
	}
