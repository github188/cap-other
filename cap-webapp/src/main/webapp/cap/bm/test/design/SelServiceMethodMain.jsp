...
function loadNode(node) {
	...
	ServiceObjectFacade.queryServiceMethodForNode(node.getData().key,function(data){
		node.addChild(data);
...
function nodeUrl(nodeId){
	var url = "ServiceMethodSelList.jsp?serviceModelId=" + nodeId + "&nodeType=app";
...
function treeClick(node){
	...
	var nodeType = "app";
	if(data.data.modelType  && data.data.modelType == "serve"){
		nodeType = "serve";
	}
	var url = "ServiceMethodSelList.jsp?serviceModelId=" + data.data.modelId + "&nodeType=" + nodeType;
	cui('#body').setContentURL("center", url);	
}
