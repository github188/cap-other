...
<body ng-controller="customEditableGridTheadCtrl" data-ng-init="ready()">
			                        ...
			                        <tbody>
			                            <tr ng-repeat="customHeaderVo in customHeaders track by $index" style="background-color: {{customHeaderVo.check == true ? '#99ccff':'#ffffff'}}">
			                                ...
			                                <td style="text-align:left;cursor:pointer" ng-click="customHeaderTdClick(customHeaderVo)">
			                                   {{customHeaderVo.indent}}&nbsp;{{customHeaderVo.name}}{{customHeaderVo.edittype.data.uitype != null ? '-['+customHeaderVo.edittype.data.uitype+']' : ''}}
			                                </td>
			                            </tr>
			                       </tbody>
					            </table>
	...
	<script type="text/javascript">
		...
		var edittype = window.opener.scope.data.edittype;
		var primarykey = window.opener.scope.data.primarykey;
		var pageSession = new cap.PageStorage(namespaces);
		var page = pageSession.get("page");
		var packageId = namespaces;
		...
		angular.module('customEditableGridThead', ["cui"]).controller('customEditableGridTheadCtrl', function ($scope, $compile) {
	    	...
	    	$scope.loadColumnProperties=function(){
				...
				ComponentFacade.query('req.uicomponent.common.component.editableGrid', "properties[ename='columns']", function(data){
	    	...
	    	$scope.loadComponent=function(componentName){
	    		if(componentName != null && componentName != ''){
	    			...
	    			var modelId = "req.uicomponent."+ group +".component."+componentName.substring(0, 1).toLowerCase()+componentName.substring(1);
    				...
    				var properties = _.remove($scope.component.properties, function(n) {
  					  	return n.ename != 'databind' && n.ename != 'id' && (n.hide == null || n.hide == false);
  					});
    				...
    				var events = _.remove($scope.component.events, function(n) {
  					  	return n.hide == null || n.hide == false;
  					});
    				$scope.component.events = events;
		    ...
		    $scope.openSelectAction=function(obj){
	    	   ...
	    	   var url='${pageScope.cuiWebRoot}/cap/bm/dev/page/designer/PageActionSelect.jsp?modelId='+namespaces+'&packageId='+namespaces+"&flag="+obj+"&methodTemplate="+node.methodtemplate+"&actionType="+node.actiontype+"&type=req";
			   var width=800; //窗口宽度
			   var height=650;//窗口高度
	        ...
	        on_click : function(obj){
	        	if(obj.id === 'batchInsertCol'){
	        		...
	        		if(obj.id === 'insertSerialCol'){
		        		scope.addCustomHeader({customHeaderId: (new Date()).valueOf() + '', name: '序号', sort: 'false', hide: 'false', disabled: 'false', level: 1, indent:'', bindName:'1', edittype:{data:{}}});
		        	} else if(obj.id === 'insertBlankCol'){
		        		scope.addCustomHeader({customHeaderId: (new Date()).valueOf() + '', name: '', sort: 'false', hide: 'false', disabled: 'false', level: 1, indent:'', edittype:{data:{}}});
		        	} else if(obj.id === 'insertOperationCol'){
		        		scope.addCustomHeader({customHeaderId: (new Date()).valueOf() + '', name: '行操作', sort: 'false', hide: 'false', disabled: 'false', level: 1, indent:'', edittype:{data:{}}});
		        	}
		        	cap.digestValue(scope);
	        	}
	        }
		...
		function openRenderWindow(event, self){
			...
			var url='CustomGridColumnsRender.jsp?reqFunctionSubitemId=' + reqFunctionSubitemId + '&namespaces=' + namespaces + '&callbackMethod=openWindowCallback&bindName='+bindName;
	 	    ...
	 	    for(var i in scope.customHeaders){
			    ...
			    if(!$.isEmptyObject(validDataList)){
			    	_.forEach(validDataList, function (value, key) {
			    		...
			    		//grid中id属性不需要进行验证
			    		if(key !== 'id' && key !== 'databind' && cui("#"+key).isCUI){
		...
		*/
		...
		//直接新增行为
	   	function openAddAction(obj){
	   		 var node = cui('#'+obj).options;
	   		 var url='${pageScope.cuiWebRoot}/cap/bm/dev/page/designer/PageActionEdit.jsp?modelId='+packageId+'&packageId='+packageId+"&flag="+obj+"&operationType=insert&methodTemplate="+node.methodtemplate+"&actionType="+node.actionType+"&type=req";
			 var width=850; //窗口宽度
			 var height=650;//窗口高度
			 var top=(window.screen.height-30-height)/2;
			 var left=(window.screen.width-10-width)/2;
	    ...
	    function openEditAction(obj){
	    	 ...
	    	 var url='${pageScope.cuiWebRoot}/cap/bm/dev/page/designer/PageActionEdit.jsp?modelId='+packageId+'&packageId='+packageId+"&flag="+obj+"&operationType=edit&methodTemplate="+node.methodtemplate+"&actionType="+node.actionType+"&selectActionId="+pageActionId+"&type=req";
			 var width=850; //窗口宽度
			 var height=650;//窗口高度
			 var top=(window.screen.height-30-height)/2;
			 var left=(window.screen.width-10-width)/2;
