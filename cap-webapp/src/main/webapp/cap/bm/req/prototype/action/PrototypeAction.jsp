...
<body style="background-color:#f5f5f5;" ng-controller="actionListCtrl" data-ng-init="ready()">
			        	...
			        	<table class="cap-table-fullWidth">
						    ...
						    <tr>
						    	<td style="text-align: left;">
									...
									<table ng-if="selectPageActionVO.actionDefineVO.modelId!=null" class="cap-table-fullWidth" cui_property exclude="actionType" id="newProperty" ng-model="selectPageActionVO"></table>
									<table class="cap-table-fullWidth">
									    <tr>
	...
	<script type="text/javascript">
		...
		angular.module('actionList', ["cui"]).controller('actionListCtrl', function ($scope, $timeout) {
			...
			$scope.$watch("codemirrors",function(){
		   	...
		   	$scope.copyPageAction=function(){
			    for(var i=0, len=$scope.root.pageActions.length; i<len; i++){
			...
			$scope.openSelectActionTmpWin=function(){
				var url='${pageScope.cuiWebRoot}/cap/bm/dev/page/actionlibrary/ActionTemplateList.jsp?callbackMethod=selectActionTmp&type=req';
				var width=800; //窗口宽度
			    var height=600;//窗口高度
		...
		//打开实体服务选择界面
		function openEntityMethodSelectWindow(isGenerateParameterForm,isTree,actionReLoadMethodName) {
			...
			var url = "${pageScope.cuiWebRoot}/cap/bm/dev/page/designer/EntityServiceSelect.jsp?packageId="+packageId+"&isGenerateParameterForm="+b;
			if(isTree){
				url = "${pageScope.cuiWebRoot}/cap/bm/dev/page/designer/EntityServiceSelect.jsp?packageId="+packageId+"&isGenerateParameterForm="+b+"&isTree=true"+"&actionReLoadMethodName="+f;
			}
			if(f!=false){
		...
		//选择实体方法回调
		function entityServiceSelectCallBack(data,isGenerateParameterForm,isTree,actionReLoadMethod) {
		...
		function openDataStoreSelect(flag,isWrap) {
			// var url = 'DataStoreSelect.jsp?packageId=' + packageId+"&modelId="+pageId+"&flag="+flag+"&isWrap="+isWrap;
			...
			var url;
			if(flag == 'pageURL' ||flag=="forwardUrl" ) {
				url = webPath + '/cap/bm/req/prototype/design/uilibrary/SelectUrl.jsp?reqFunctionSubitemId=' + reqFunctionSubitemId + '&callbackMethod=pageSelectCallBack&propertyName=' + flag + '&value=' + scope.selectPageActionVO.methodOption.pageURL;
			}
			var top=(window.screen.availHeight-600)/2;
			var left=(window.screen.availWidth-800)/2;
		...
		function pageSelectCallBack(propertyName, value) {
			...
			scope.selectPageActionVO.methodOption[propertyName] = '"' + cap.getRelativeURL(value.url, page.modelPackage) + '"';
			cap.digestValue(scope);
		}
		...
		//打开页面控件选择页面
		function openComponentSelect(selectMode){
			var top=(window.screen.availHeight-600)/2;
			var left=(window.screen.availWidth-500)/2;
			window.open ('${pageScope.cuiWebRoot}/cap/bm/dev/page/designer/PageComponentSelect.jsp?pageId='+pageId+"&selectMode="+selectMode,'pageComponentSelect','height=500,width=600,top='+top+',left='+left+',toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no') 
		}
