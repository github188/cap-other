...
<!DOCTYPE html>
<html ng-app='customEditableGrideditType'>
<head>
	<title>editType属性</title>
...
<body ng-controller="customEditableGrideditTypeCtrl" data-ng-init="ready()">
<div>
	...
	<script type="text/javascript">
		var namespaces = "<c:out value='${param.namespaces}'/>";
		var reqFunctionSubitemId = "<c:out value='reqFunctionSubitemId}'/>";
		...
		//公用开发建模代码，变量做冗余
		var pageId = namespaces, packageId = page.packageId;
		...
		angular.module('customEditableGrideditType', ["cui"]).controller('customEditableGrideditTypeCtrl', function ($scope, $compile) {
	    	//数据模型属性（左侧表格数据源）
		    ...
		    $scope.initCulomnsBindNames=function(columnVo, id){
		    	if(columnVo.bindName != null){
    				...
    				var culomnsBindNameVo = {id: id, name: columnVo.name, bindName: columnVo.bindName};
			...
			$scope.save=function(){
	    		...
	    		for(var i in culomnsBindNames){
	    			...
	    			var editType = culomnsBindName.data;
	    			if(editType != null){
	    				//剔除属性值为空或null的值
	    				$scope.filterBlankAndEmpty(editType);
	    				var propertiesType = culomnsBindName.propertiesType;
	    	    		//根据控件属性类型转换
	    	    		for(var key in propertiesType){
	        				var type = propertiesType[key];
	        				if(editType[key] == null){
	        					continue;
	        				}
	        				if(type === 'Number'){
	        					editType[key] = Number(editType[key]);
	        				} else if(type === "Boolean"){
	        					editType[key] = editType[key] === "true" ? true : false;
	        				} else if(type === "Json"){
	        					try{
	        						editType[key] = eval("("+ editType[key] +")");
	        					} catch (e){
	        						editType[key] = editType[key];
	        					}
	        				} else {
	        					editType[key] = editType[key];
	        				} 
	        			}
	    	    		var hasThrdUI = hasThrdComponent(editType.uitype);
		    			if(hasThrdUI){
		    				editType.thrdui = true;
		    			}
	    				jsonToEditType += '"'+culomnsBindName.bindName+'":'+JSON.stringify(editType)+',';
	    			}
	    		}
	    		if(jsonToEditType != ''){
	    			jsonToEditType = "{"+jsonToEditType.substring(0,jsonToEditType.length-1)+"}";
	    		}
	    		window.opener[callbackMethod](propertyName, jsonToEditType);
	    		//importIncludeFileList(jsonToEditType != '' ? eval("("+jsonToEditType+")") : {}, pageSession.get("page"));//根据控件定义类型是否用人员或组织、数字字典，引用相关文件(js\css)
	    		window.close();
			};
	    	...
	    	$scope.$watch("uitype", function(newValue, oldValue){
	    		if(newValue != null && newValue != ""){
	    			if($scope.culomnsBindNameVo.data != null && $scope.culomnsBindNameVo.data.uitype != newValue){//同一个列表更改控件定义类型，需要重新初始化$scope.data.edittype.data
