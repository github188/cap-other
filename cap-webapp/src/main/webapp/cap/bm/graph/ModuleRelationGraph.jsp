...
<head>
    ...
    <script type="text/javascript" src="<%=request.getContextPath() %>/cap/bm/graph/js/moduleHelp.js"></script>
	...
	<script type="text/javascript">
		...
		function main(container)
		{
            ...
            graph.addListener(mxEvent.DOUBLE_CLICK, function(sender, evt)
            {
            	...
            	//为了让依赖的外部图形显示 id修改为 targetID_sourceId
            	if(cellId.indexOf('_')>=0){
            		cellId = cellId.split('_')[0];
            	}
            	var mName;
            	dwr.TOPEngine.setAsync(false);
            	GraphAction.queryGraphModuleNameByModuleId(cellId, function(data) {
            		mName = data.moduleName;
            	});
            	dwr.TOPEngine.setAsync(true);
                if(isFolder(cell)){
                	//打开模块关系图
                	parent.cui("#moduleRelationTab").addTab({id:'tab_' + cellId, title:mName,url: 'ModuleRelationGraph.jsp?moduleId=' + cellId, closeable: true});
                	parent.cui("#moduleRelationTab").switchTo(parent.cui("#moduleRelationTab").contents.length - 1);
                }else{
                	//打开应用内资源关系图
                	var tabTitle = mName + "资源关系";
                	parent.cui("#moduleRelationTab").addTab({id:'tab_' + cellId, title:tabTitle,url: 'ClassRelationGraph.jsp?moduleId=' + cellId, closeable: true});
                	parent.cui("#moduleRelationTab").switchTo(parent.cui("#moduleRelationTab").contents.length - 1);
                }
    ...
    <script id="componentDiagram" type="text/x-dot-template">
        <mxGraphModel grid="1" guides="1" tooltips="1" connect="1" fold="1" page="0" pageScale="1" pageWidth="100%" pageHeight="100%">
            <root>
    ...
    <script id="centerContanierNodeDiagram" type="text/x-dot-template">
		...
		<mxCell id="{{=centerContanierNode.id}}" value="&lt;div title='{{=moduleNameFullPath}}' 
			style='width:{{=centerContanierNode.width-30}}px;height:20px;line-height:20px;overflow:hidden;word-wrap:normal;white-space:nowrap;text-overflow:ellipsis;' &gt;{{=moduleNameFullPath}}&lt;/div&gt;" style="swimlane;fillColor=#FFFFFF;gradientColor=#9EA4F5;strokeColor=#B9C2F5;html=1" vertex="1" parent="1">
		...
		<mxCell id="{{=node.moduleId}}" value="&lt;table title='{{=nodeModuleNameFullPath}}' style='width: 60px;height: 53px;border:0;cellpadding:0;cellspacing:0;'&gt;&lt;tr&gt;&lt;td style='word-break:break-all;white-space:pre-wrap;vertical-align: middle; ' &gt;{{=node.moduleName}}&lt;/td&gt;&lt;/tr&gt;&lt;/table&gt;" style="shape=component;align=left;spacingLeft=35;fillColor=#FFFFFF;gradientColor=#D9EAFF;strokeColor=#64b4e6;html=1" vertex="1" parent="{{=centerContanierNode.id}}">
      		<mxGeometry x="{{=node.X}}" y="{{=node.Y}}" width="{{=node.width}}" height="{{=node.height}}" as="geometry"/>
    	</mxCell>
		{{
				} else {
        }}
		<mxCell id="{{=node.moduleId}}" value="&lt;table title='{{=nodeModuleNameFullPath}}' style='margin-top:6px;width:85px;height: 45px;border:0;cellpadding:0;cellspacing:0;' &gt;&lt;tr&gt;&lt;td style='word-break:break-all;white-space:pre-wrap;vertical-align: middle;' &gt;{{=node.moduleName}}&lt;/td&gt;&lt;/tr&gt;&lt;/table&gt;" style="shape=folder;fontStyle=1;spacingTop=0;spacingLeft=0;spacingRight=0;spacingBottom=0;tabWidth=40;tabHeight=14;tabPosition=left;fillColor=#FFFFFF;gradientColor=#F2F789;strokeColor=#DFD89D;html=1" vertex="1" parent="{{=centerContanierNode.id}}">
      		<mxGeometry x="{{=node.X}}" y="{{=node.Y}}" width="{{=node.width}}" height="{{=node.height}}" as="geometry"/>
    	</mxCell>
		{{
				}
    ...
    <script id="contanierNodeDiagram" type="text/x-dot-template">
		...
		<mxCell id="{{=node.moduleId}}" value="&lt;table title='{{=nodeModuleNameFullPath}}' style='width: 60px;height: 53px;border:0;cellpadding:0;cellspacing:0;'&gt;&lt;tr&gt;&lt;td style='word-break:break-all;white-space:pre-wrap;vertical-align: middle;' &gt;{{=node.moduleName}}&lt;/td&gt;&lt;/tr&gt;&lt;/table&gt;" style="shape=component;align=left;spacingLeft=35;fillColor=#FFFFFF;gradientColor=#D9EAFF;strokeColor=#64b4e6;html=1" vertex="1" parent="{{=(contanierNode.nodes.length > 1 ? contanierNode.id : 1)}}">
      		<mxGeometry x="{{=node.X}}" y="{{=node.Y}}" width="{{=node.width}}" height="{{=node.height}}" as="geometry"/>
    	</mxCell>
		{{
				} else {
        }}
		<mxCell id="{{=node.moduleId}}" value="&lt;table title='{{=nodeModuleNameFullPath}}' style='margin-top:6px;width: 85px;height: 45px;border:0;cellpadding:0;cellspacing:0;'&gt;&lt;tr&gt;&lt;td style='word-break:break-all;white-space:pre-wrap;vertical-align: middle;' &gt;{{=node.moduleName}}&lt;/td&gt;&lt;/tr&gt;&lt;/table&gt;" style="shape=folder;fontStyle=1;spacingTop=10;tabWidth=40;tabHeight=14;tabPosition=left;fillColor=#FFFFFF;gradientColor=#F2F789;strokeColor=#DFD89D;html=1" vertex="1" parent="{{=(contanierNode.nodes.length > 1 ? contanierNode.id : 1)}}">
      		<mxGeometry x="{{=node.X}}" y="{{=node.Y}}" width="{{=node.width}}" height="{{=node.height}}" as="geometry"/>
    	</mxCell>
		{{
				}
...
<body onload="main(document.getElementById('graphContainer'))">
	...
	<div id="graphContainer" class="geDiagramContainer"
		style="width:100%;">
		<h1 id="blank_tip" style="position:absolute; top:40%; left:30%; height:20%;display:none">该模块无数据,无法显示！</h1>
...
function exportToImageByNative(){
	var svg = $("svg:first")[0];
	var fileName = "模块关系图-" + moduleName + ".png";
	saveSvgAsPng(svg, fileName);
}

function exportToImage(){
	if (isChrome) {
		exportToImageByNative();
	} else {
		exportToImageByMxgraph();
	}
}

function exportToImageByMxgraph(){
