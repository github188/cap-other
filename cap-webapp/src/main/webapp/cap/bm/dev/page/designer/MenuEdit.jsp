		...
		<div>
				<table class="form_table">
						...
						<tr id="funcCodeTr">
							...
							<td class="td_content" width="20%">
								<span uitype="input" id="funcCode" name="funcCode" databind="funcData.funcCode"  width="200" maxlength="40"
...
<script type="text/javascript">
	...
	$(document).ready(function (){
		...
		comtop.UI.scan();
		if(funcNodeType != 2){
			window.validater.disValid('funcTag', true);
		}else{
			$('#funcUrlSpan').show();
		...
		if(inValid.length > 0){//验证失败
			...
			if(actionType == "add"){
				...
				FuncAction.saveFunc(vo, function(data){
					funcId = data;
					var newFuncVO={funcId:"",funcName:"",funcCode:"",description:"",attributeValue:""};
					newFuncVO.funcId = data;
					newFuncVO.funcName = vo.funcName;
					newFuncVO.funcCode = vo.funcCode;
					newFuncVO.description = vo.description;
					window.opener.addRightsVariableBack(newFuncVO);
					window.close();
				});
			}else{
				...
				FuncAction.updateFunc(vo, function(data){
					window.opener.addRightsVariableBack(data);
				});
				dwr.TOPEngine.setAsync(true);
			}
		}
