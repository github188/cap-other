...
<div>
	        		...
	        		<table class="cap-table-fullWidth" style="width: 100%;" ng-if="render == 'a'">
	                    <tr>
	                        ...
	                        <td class="cap-td" style="text-align:left;">
	                    		<span cui_clickinput id="title" name="title" ng-click="openSelectUrl('title')" ng-model="options.url" width="100%"></span>
	                         </td>
	                    </tr>
	                    <tr>
	                        ...
	                        <td class="cap-td" style="text-align:left;">
                            	<span cui_clickinput id="params" name="params" ng-model="options.params" width="100%" on_iconclick="openEntityAttributesWindow"></span>
	                        </td>
	                    </tr>
	                    <tr>
	                        <td class="cap-td" style="text-align:right;">
	...
	<script type="text/javascript">
		...
		var modelId = window.opener.pageId;
		...
		angular.module('customGridColumnsRender', ["cui"]).controller('customGridColumnsRenderCtrl', function ($scope, $compile) {
			...
			$scope.save=function(){
				...
				for(var key in $scope.options){
    				...
    				if($scope.options[key] != '' && ((key === 'url' && $scope.render != 'image') || key === 'compare' || key === 'title')){
	    	...
	    	$scope.openSelectAction=function(obj){
	    		 var url='${pageScope.cuiWebRoot}/cap/bm/dev/page/designer/PageActionSelect.jsp?modelId='+window.opener.pageId+'&packageId='+window.opener.packageId+"&flag="+obj;
				   var width=800; //窗口宽度
				   var height=550;//窗口高度
	    	...
	    	$scope.openSelectUrl=function(obj){
	    		var url = '${pageScope.cuiWebRoot}/cap/bm/dev/page/designer/DataStoreSelect.jsp?packageId=' + window.opener.packageId+"&modelId="+window.opener.pageId+"&selectType=url&flag="+obj;
	    		var top=(window.screen.availHeight-600)/2;
	    		var left=(window.screen.availWidth-800)/2;
	    		openWindow(url,'openDataStoreSelect','height=600,width=800,top='+top+',left='+left+',toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no')
	    	}
		});
		 ...
		 * @param self
		 */
		function openEntityAttributesWindow(event, self){
			var width=800; //窗口宽度
		    var height=600; //窗口高度
		    var top=(window.screen.height-30-height)/2;
		    var left=(window.screen.width-10-width)/2;
		    var propertyName=self.options.name;
			var url='EntityAttributesSelect.jsp?propertyName='+propertyName+'&callbackMethod=openWindowCallback';
			openWindow(url, "entityAttributes", "Scrollbars=no,Toolbar=no,Location=no,titlebar=no,Direction=no,Resizeable=no,alwaysLowered=yes,Width="+width+" ,Height="+height+",top="+top+",left="+left);
		}
