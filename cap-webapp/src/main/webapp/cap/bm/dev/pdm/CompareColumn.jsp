...
<body style="background-color:#f5f5f5;" ng-controller="compareColumnController" data-ng-init="ready()">
<div class="cap-page">
    <div class="cap-area" style="width:100%;">
		...
		<table class="custom-grid" style="width: 100%">
			<thead>
			   <tr>
				   <th style="width:30px;">
				      <input type="checkbox" ng-change="checkAllChange(checkAll,datas)" ng-model="checkAll">
				   </th>
...
<script type="text/javascript">
	 ...
	 angular.module('compareColumn', [ "cui"]).controller('compareColumnController', function($scope) {
			...
			//页面初始化
			$scope.init =function(){
				if(datastorage["columnSize"]>0){
				   $scope.datas=datastorage["columns"];
				   $scope.selectedData = datastorage["columns"][0];
				}
			};
			...
			$scope.sysncSelected = function(){
				...
				$scope.selectedDatas.forEach(function(item, index, arr) {
					tableCompareResults.push(item["columnCompareResult"]);
				});

                //同步列
				$scope.sysncColumn(tableCompareResults);
			};
            ...
            //同步列结构差异
			$scope.sysncColumn = function(tableCompareResults){
				$scope.disablebutton();

				dwr.TOPEngine.setAsync(false);
				TableOperateAction.sysncColumn(tableCompareResults, packageId, {
            ...
            //存储列信息
			$scope.storageColumns = function(){
				datastorage["selectedColumns"] = $scope.selectedDatas;
			}
	 });
