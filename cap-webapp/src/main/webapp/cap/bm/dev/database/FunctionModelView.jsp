...
<head>
<title>函数信息页面</title>
	<link rel="shortcut icon" href="${pageScope.cuiWebRoot}/cap/ptc/index/image/function.png">
...
<body>
	<div class="top_header_wrap">
		<div class="thw_title">数据库函数信息</div>
	...
	<div id="editDiv"  class="top_content_wrap cui_ext_textmode" >
		<table class="form_table" style="table-layout:fixed;">
			...
			<tr>
				<td class="td_label">函数名：</td>
				<td>
					<span uitype="input" id="engName" name="engName" databind="functionData.engName" width="100%"  readonly="true">
				</td>
				<td class="td_label">中文名：</td>
				<td>
					<span uitype="input" id="chName" name="chName"  width="100%" databind="functionData.chName"  >
				</td>
			</tr>
			<tr>	
				<td class="td_label" valign="top" style="padding-top: 5px;">描述：</td>
				<td >
					<span uitype="textarea" id="description" maxlength="-1" databind="functionData.description" 
		...
		<table uitype="Grid" id="FunctionColumnGrid" primarykey="parameterName" selectrows="no" sorttype="DESC" datasource="initData" pagination="false"  pagesize_list="[10,20,30]"  
	...
	function init() {
		...
		FunctionFacade.queryFunctionById(modelId,function(data){
			functionData = data;
		});
	...
	function initData(tableObj,query){
		if(inited) {
			tableObj.setDatasource(functionData.functionColumns,functionData.functionColumns.length);
		}
	}
	
	// 字段类型
// 	var fieldTypeData = ["FunctionColumnUnknown","FunctionColumnIn","FunctionColumnInOut","FunctionColumnOut","FunctionColumnReturn","FunctionColumnResult"];
	...
	function back() {
		window.location.href = "FunctionModelList.jsp?packageId=${param.packageId}"+"&packagePath="+packagePath;
	}
	//grid 宽度
	...
	function save(){
		dwr.TOPEngine.setAsync(false);
		FunctionFacade.saveFunction(functionData,function(data){
			  if(data){
...
// 		cui("#TableColumnGrid").selectRowsByPK(rd.parameterName,true);
	...
	function updateParameterChName(parameterName,id) {
		for(var i= 0 ; i <  functionData.functionColumns.length; i++) {
			if((id && id ==  functionData.functionColumns[i].id) || parameterName ==  functionData.functionColumns[i].parameterName) {
				objectVO = functionData.functionColumns[i];
				break;
			}
		}
	...
	//编辑参数回调
 	function editParameterBack(pVo) {
 		if(functionData.functionColumns && functionData.functionColumns.length) {
	 		for(var i= 0 ; i <  functionData.functionColumns.length; i++) {
	 			var o1 = functionData.functionColumns[i];
	 			if((o1.id != null && o1.id == pVo.id)|| o1.parameterName == pVo.parameterName) {
	 				functionData.functionColumns[i] = pVo;
	 				break;
	 			}
			}
	 		//刷新页面
	 		cui("#FunctionColumnGrid").loadData();
 		}
	}
