...
<div uitype="Borderlayout"  id="body"  is_root="true"> 
	...
	<div id="centerMain" position="center" width="500" collapsabltie="true" show_expand_icon="true">       
        <table width="85%" style="margin-left: 10px">
			<tr height="40px;">
				<td class="fontTitle" width="250px">由数据库导入函数</td>
			</tr>
			<tr height="40px;">
				<td align="right">
					<span uitype="ClickInput" id="keyword" name="keyword" emptytext="请输入函数名过滤"
			...
			<tr>
				<td>
					 <table uitype="Grid" id="FunctionImportGrid" primarykey="engName" datasource="initData" pagination="false"  pagesize_list="[10,20,30]"  
					 	pagesize="20" resizewidth="resizeWidth" resizeheight="resizeHeight" >
					 	<thead>
					 	<tr>
							<th style="width:50px"><input type="checkbox"/></th>
							<th bindName="engName" style="width:50%;" renderStyle="text-align: left;" >函数名</th>
...
var packagePath = "${param.packagePath}";//包路径
	...
	function fastQuery() {
		initData(cui("#FunctionImportGrid"),cui("#FunctionImportGrid").getQuery());
	}
	...
	//导入
	function enSure() {
		var selectData = cui("#FunctionImportGrid").getSelectedRowData();
		...
		setTimeout(function(){
			FunctionFacade.importFunctions(packagePath,selectData,function(data){
				if(data) {
					window.opener.focus();
					window.opener.cui.message("导入函数成功。","success");
					window.opener.importCallback();
					//window.opener.cui("#FunctionGrid").loadData();
					closeWin();
				}else {
					window.cui.message("导入函数出错，请联系管理员。");
				}
				cui.handleMask.hide();
			});
		}, 100);
