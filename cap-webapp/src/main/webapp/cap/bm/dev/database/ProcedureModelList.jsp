...
<div class="list_header_wrap">
	<div class="top_float_left">
		<div class="thw_title" style="margin-left:11px;margin-top:0px;">
			<font id="pageTittle" class="fontTitle">存储过程模型列表信息</font> 
		</div>
	</div>
	<div class="top_float_right">
		<span uitype="button" label="导入" id="button_add" on_click="importEntity"></span>
		<span uitype="button" label="删除" id="button_del" on_click="delProcedure"></span>
	</div>
</div>
 <table uitype="Grid" id="ProcedureGrid" primarykey="modelId"  sorttype="DESC" datasource="initData" pagination="false"  pagesize_list="[10,20,30]"  
 	...
 	<tr>
		...
		<th bindName="engName" style="width:30%;" renderStyle="text-align: left;"  render="correspondTableRenderer">存储过程名称</th>
...
var packageModuleCode = "${param.packageModuleCode}";//父模块编码
	...
	//删除
	function delProcedure(){
		var selectIds = cui("#ProcedureGrid").getSelectedPrimaryKey();
		if(selectIds == null || selectIds.length == 0){
			cui.alert("请选择要删除的存储过程。");
			return;
		}
		cui.confirm("确定要删除这"+selectIds.length+"个存储过程吗？",{
			onYes:function(){
				dwr.TOPEngine.setAsync(false);
				ProcedureFacade.delProcedures(selectIds,function(data){
					if(data){
						cui.message("删除成功。","success");
					}else{
						cui.message("删除失败。","error");
					}
		 			cui("#ProcedureGrid").loadData();
		 		});
		 		dwr.TOPEngine.setAsync(true);
			}
		});
	...
	function viewTableModel(modelId) {
		var url =  "ProcedureModelView.jsp?packageId=${param.packageId}&modelId=" + modelId +"&packagePath="+packagePath+"&openType=listToMain";
	...
	function importEntity() {
		var url = "ProcedureImport.jsp?packageId=" + packageId  +"&packagePath="+packagePath;
		try {
