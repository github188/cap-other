		...
		<table class="custom-grid" style="width: 100%">
		    ...
		    <tbody>
	           <tr ng-repeat="column in tableData.columns" style="background-color: {{$index == selectedIndex ? '#99ccff' : '#ffffff'}}" ng-click="rowClick(column,$index)">
	                 ...
	                 <td style="text-align: left;" class="notUnbind">
	                      <span>
	                      	<input type="number" class="input-shadow" min="1" max="{{column.dataType=='NUMERIC' ? '38':'' }}" ng-keydown="keydown()" ng-class="column.length==null || column.length<0 ? 'input-required':''" ng-model="column.length" ng-change="lengChange(column.length,column.dataType,column)" style="{{(column.dataType=='TIMESTAMP' || column.isPrimaryKEY || column.dataType=='BLOB' || column.dataType=='TEXT' || column.dataType=='INT') ? 'background: #f5f5f5;':''}}" ng-disabled="column.dataType=='TIMESTAMP' || column.isPrimaryKEY || column.dataType=='BLOB' || column.dataType=='TEXT' || column.dataType=='INT'">
	                      </span>
	                 </td>
	                 <td style="text-align: left;" class="notUnbind" >
	                      <span>
	                      	<input type="number" class="input-shadow" ng-keydown="keydown()" min="0" ng-class="column.precision==null || column.precision<0 ? 'input-required':''" ng-model="column.precision" style="{{(column.dataType!='NUMERIC') ? 'background: #f5f5f5;':'' }}" ng-disabled="column.dataType!='NUMERIC' ">
	                      </span>
	                 </td>

	                 <td style="text-align: left;" class="notUnbind" >
	                      <span>
	                      	<input type="number" class="input-shadow" ng-if="column.dataType=='NUMERIC' || column.dataType=='INT'" min="0" ng-model="column.defaultValue" style="{{(column.isPrimaryKEY) ? 'background: #f5f5f5;':''}}" ng-disabled="column.isPrimaryKEY">
	                      	<input type="text" class="input-shadow" ng-if="column.dataType!='NUMERIC' && column.dataType!='TIMESTAMP' && column.dataType!='INT'" ng-model="column.defaultValue" style="{{(column.isPrimaryKEY || column.dataType=='BLOB' || column.dataType=='TEXT') ? 'background: #f5f5f5;':''}}" ng-disabled="column.isPrimaryKEY || column.dataType=='BLOB' || column.dataType=='TEXT'">
  	                      	<span cui_pulldown id="defaultValue" ng-if="column.dataType=='TIMESTAMP'" ng-model="column.defaultValue" editable="false" value_field="id" label_field="text" width="100%">
  								<a value="">无</a>
  								<a value="CURRENT_TIMESTAMP">系统日期</a>
  							 </span>
	                      </span>
	                 </td>

	                 <td style="text-align:center;" class="notUnbind" >
						 <input type="checkbox" ng-model="column.canBeNull" ng-disabled="column.isPrimaryKEY" ng-change="canBeNullChange(column,column.dataType,column.canBeNull)">
	                 </td>
	                 <td style="text-align: left;" class="notUnbind" >
	                      <span>
...
angular.module('tableModel', ["cui"]).controller('tableModelController', function($scope) {
	...
	/**
	 * 如果数据类型为NUMERIC类型默认值转换为NUMERIC类型,解决angular js转换问题
	 * 
	 * @return {[type]} [description]
	 */
	...
	$scope.validateColumns = function() {
		...
		for (var i = 0; i < $scope.tableData.columns.length; i++) {
			...
			if (column.dataType == "NUMERIC" && column.precision > column.length) {
				cui.alert("NUMERIC类型精度不能大于长度.");
				return false;
			}
	...
	$scope.canBeNullChange = function(column, dataType, canBeNull) {
		if (dataType == "TIMESTAMP" && !canBeNull) {
