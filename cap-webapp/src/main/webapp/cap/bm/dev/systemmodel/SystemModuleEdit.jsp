...
<div uitype="Borderlayout"  id="body" is_root="true">	
	<div class="top_header_wrap" style="padding-top:3px">
		<div class="thw_title">
			<font id = "pageTittle" class="fontTitle">系统信息编辑</font> 
		</div>
		...
		<!-- <span uitype="button" id="import_solution" label="导入解决方案"  on_click="importSolution" ></span> -->
		</div>
	...
	<div class="cui_ext_textmode" >
		<table class="form_table_style" style="table-layout:fixed;">
	        ...
	        <tr>
	            <td class="td_label"> <span id="requiredName" class="top_required">*</span>系统名称：</td>
	            <td>
	               <span uitype="input" id="moduleName" name="moduleName" databind="systemData.moduleName" maxlength="36" emptytext="请输入..."
	                validate="validateModuleName">
	                </span>
	            </td>
	            </tr>
	            <tr>
	        	<td class="td_label"><span id="requiredModuleCode" class="top_required">*</span>系统编码：</td>
				<td>
					<span uitype="input" id="moduleCode" name="moduleCode" databind="systemData.moduleCode" maxlength="36" emptytext="请输入..."
	               validate="validateModuleCode"></span>
				</td>
	        </tr>
	        
	       <!--  <tr>
				<td class="td_label" width="25%">项目类型 ：<span style="color: red">*</span></td>
				<td>
					<span id="projectType" uitype="RadioGroup" name="projectType" databind="data.projectType" value="0">
			...
			<!-- <tr>
				...
				<td>
					<span uitype="input" id="shortName" name="shortName" databind="systemData.shortName" maxlength="15" emptytext="请输入..." validate="validateShortName"></span>
					<span id="shortNamePS" name="shortNamePS"><font style="color: #B5B5B5">&nbsp;用于生成数据库表前缀，不区分大小写，如：fwms</font></span>
				</td>
	        </tr>
	         -->
	        <tr id="tr_parent">
				...
				<td>
				<span uitype="input" name="parentName" id="parentName" databind="systemData.parentName" width="440px" readonly="true"></span>
				<span uitype="input" id="parentModuleId" name="parentModuleId" databind="systemData.parentModuleId" readonly="true"></span>	
				</td>
			</tr>
			...
			<tr>
				...
				<td class="td_content" >
					<div style="width:440px;">
						<span uitype="textarea" id="description" name="description" databind="systemData.description" 
	...
	<script type="text/javascript">
		...
		var isRootNodeAdd = "${param.isRootNodeAdd}";
		...
		function initFuncItemData(tableObj,query){
			...
			SystemModuleAction.queryReqTreeVOByModuleId(systemData.moduleCode,function(res){
		...
		*/
		...
		$(document).ready(function() {
			...
			if(actionType == 'add') { // 新增 
				...
				SystemModelAction.getModuleInfo(nodeId,function(moduleData){
					cui(systemData).databind().setValue(moduleData);
					comtop.UI.scan();   //扫描
				});
				dwr.TOPEngine.setAsync(true);
		...
		function initEditPage() {
			...
			cui('#parentModuleId').hide();
			cui('#parentName').setValue(decodeURIComponent(parentNodeName));
			cui('#back_to').hide();
		...
		function save() {
			...
			if(inValid.length > 0){//验证失败
				...
				//验证全局编码唯一性 
				var moduleVO = cui(systemData).databind().getValue();
				moduleVO.moduleType = 1;
				if(isRootNodeAdd == "true"){
				...
				if(insertFlag == 1){//新增 
					...
					SystemModelAction.insertModuleVO(moduleVO,function(data){
						if(data && data.moduleId) {
							...
							SystemModuleAction.saveModuleFuncItem(moduleVO,cui("#FuncItemGrid").getData(),function(res){
								if(res){
									window.parent.cui.message('系统及功能项新增成功。',"success");
								}else{
					...
					SystemModelAction.updateModuleVO(moduleVO,function(data){
						if(data && data.moduleId){
							...
							SystemModuleAction.saveModuleFuncItem(moduleVO,cui("#FuncItemGrid").getData(),function(res){
								if(res){
									window.parent.cui.message('系统及功能项修改成功。',"success");
								}else{
		...
		function saveRootNode(moduleVO) {
			...
			SystemModelAction.insertModuleVO(moduleVO,function(data){
				if(data && data.moduleId) {
					...
					window.parent.location .href = "SystemModelMain.jsp";
		...
		function canDeleteModuleVO(){
			var vo = {moduleId:nodeId,moduleCode:systemData.moduleCode,parentModuleId:parentNodeId};
			var moduleVO = cui(systemData).databind().getValue();
				...
				onYes: function(){
					...
					SystemModelAction.deleteModuleVO(vo,function(){
						...
						SystemModuleAction.deleteModuleFuncItem(vo,function(res){
							if(res){
								window.parent.cui.message('系统及功能项删除成功。',"success");
							}else{
								window.parent.cui.message('系统及功能项删除失败。',"error");
							}
						});
						dwr.TOPEngine.setAsync(true);
					});
		...
		function insertSubSys() {
			...
			cui(systemData).databind().setEmpty();
			funcItemClean();
		...
		//新增下级目录或应用 
		function insertSubDirOrApp(nodeType) {
			var moduleNameVal = cui('#moduleName').getValue();
			...
			if(nodeType == 3) {
				window.parent.cui('#body').setContentURL("center","<%=request.getContextPath() %>/cap/bm/dev/systemmodel/DirectoryEdit.jsp?actionType=add&parentNodeId=" + nodeId + "&parentNodeName=" + moduleNameVal
						+ "&actionType=add" + "&parentModuleType=" + parentModuleType + "&backId=" + nodeId); 
			}
		}
		...
		var viewModel = {
	 	    	datasource: [{id:'viewResource',label:'查看资源图'},
				      		    {id:'viewPhysic',label:'查看(编辑)物理部署图'},
				      		    {id:'viewLogic',label:'查看(编辑)逻辑部署图'}],
		 	     	        on_click:function(obj){
		 	     	        	if(obj.id=='viewResource'){
		 	     	        		window.open("<%=request.getContextPath() %>/cap/bm/graph/ModuleRelationGraphMain.jsp?moduleId=" + nodeId  + "&testModel="+testModel + "&returnUrl=" + escape(window.location.href), "_self");
		 	     	        	}else if(obj.id=='viewPhysic'){
		 	     	        		window.open("<%=request.getContextPath() %>/cap/bm/graph/editor/IndexMain.jsp?moduleId=" + nodeId  + "&testModel="+testModel + "&diagramType=physic" + "&returnUrl=" + escape(window.location.href), "_self");
		 	     	        	}else if(obj.id=='viewLogic'){
	 	 	     	        		window.open("<%=request.getContextPath() %>/cap/bm/graph/editor/IndexMain.jsp?moduleId=" + nodeId  + "&testModel="+testModel + "&diagramType=logic" + "&returnUrl=" + escape(window.location.href), "_self");
		 	    	        	}
		 	     	        }
		 	     	    };	
		
		// 新增同级系统 
		function insertSameSys() {
			backId = nodeId;
			updateSysModule();
			insertFlag = 1;//表示新增
		...
		// 新增同级目录和应用 
		function insertSameDirOrApp(nodeType) {
			...
			if(nodeType == 2) {
				window.parent.cui('#body').setContentURL("center","<%=request.getContextPath() %>/cap/bm/dev/systemmodel/FuncModelEdit.jsp?actionType=add&parentNodeId=" + parentNodeId + "&parentNodeName=" + parentNameVal
						+ "&actionType=add" + "&parentModuleType=" + parentModuleType + "&backId=" + nodeId);  
			}
			if(nodeType == 3) {
				window.parent.cui('#body').setContentURL("center","<%=request.getContextPath() %>/cap/bm/dev/systemmodel/DirectoryEdit.jsp?actionType=add&parentNodeId=" + parentNodeId + "&parentNodeName=" + parentNameVal
						+ "&actionType=add" + "&parentModuleType=" + parentModuleType + "&backId=" + nodeId); 
			}
		}
	 	    	...
	 	    	datasource: [
	 	     	        ...
	 	     	        on_click:function(obj){
	 	     	        	if(obj.id=='insertSameSys'){
	 	     	        		...
	 	     	        		insertSameDirOrApp(3);
	 	     	        	}else if(obj.id=='insertSameApplication'){
	 	     	        		insertSameDirOrApp(2);
	 	    	        	}
	 	     	        }
	 	     	    };
