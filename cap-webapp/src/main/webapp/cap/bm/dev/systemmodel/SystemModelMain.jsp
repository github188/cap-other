...
<body>
	...
	<script type="text/javascript">
		...
		var fromTreePage = "${param.fromTreePage}";
		...
		var addData = [];
		if(fromTreePage == "true"){
			addData = [//{"title": "实体", "data": {"moduleType": 4}}, 
			               {"title": "新版实体", "data": {"moduleType": 44}}, 
	                       //{"title": "数据库", "data": {"moduleType": 5}},
	                       {"title": "新版数据库", "data": {"moduleType": 55}},
	                       //{"title": "服务", "data": {"moduleType": 6}},
	                       {"title": "服务", "data": {"moduleType": 66}},
	                      // {"title": "界面", "data": {"moduleType": 8}},
	                       {"title": "新版界面", "data": {"moduleType": 88}},
	                       {"title": "数据项维护", "data": {"moduleType": 9}},
	                       {"title": "工作流", "data": {"moduleType": 10}},
	                       {"title":"工作台配置","data":{"moduleType":666}},
	                       {"title":"测试用例","data":{"moduleType":77}}
	                       ];
		}
		
		//初始化数据 
		function initData(obj) {
			//如果是移动操作返回的页面，需要自动展开到移动节点上。
			if(moveObjId){
				SystemModelAction.getAllModuleTree(function(data){
					if(data == null || data == "") {
				    	...
				    	var moveNode = moveObjId ? obj.getNode(moveObjId) : null;
				    	if(moveNode) {	//有来源实体需要自动定位到对应实体应用方便于用户选择
				    		moveNode.activate();
							//定位到对应的选中的项
							$(".bl_box_center").scrollTop($(".dynatree-active").offset().top);
				    	}
				    	nodeUrl(moveObjId);
					}
			     });
		...
		function nodeUrl(nodeId){
			...
			if(nodeTypeVal == 2) { // 应用节点
				...
				var urlParam = 'nodeId=' + nodeId + '&parentNodeId=' + parentData.key
					+ "&parentNodeName=" + strParentName;
				if(parentData.key == "_1"){//根节点
					var contentUrl = "<%=request.getContextPath() %>/cap/bm/graph/ModuleRelationGraphMain.jsp?moduleId=" + nodeId+'&'+urlParam;
					cui('#body').setContentURL("center" , contentUrl);
				}else{
					var contentUrl = '<%=request.getContextPath() %>/cap/bm/dev/systemmodel/SystemModuleEdit.jsp?'+urlParam;
					cui('#body').setContentURL("center",contentUrl);	
				}
		...
		function treeClick(node) {
			...
			switch(nodeTypeVal) {
				case 1:
					var urlParam = 'nodeId=' + nodeId + '&parentNodeId=' + parentData.key + '&parentNodeName=' + strParentName + '&nodeTypeVal=' + nodeTypeVal + '&parentModuleType=' + parentModuleType;
					if(parentData.key == "_1"){//根节点
						url = "<%=request.getContextPath() %>/cap/bm/graph/ModuleRelationGraphMain.jsp?moduleId=" + nodeId  + "&" + urlParam;
					}else{
						url = '<%=request.getContextPath() %>/cap/bm/dev/systemmodel/SystemModuleEdit.jsp?'+urlParam;
					}
					break;
				case 2:
					if(fromTreePage == "true"){
						url = 'FuncModelEdit.jsp?nodeId=' + nodeId + '&parentNodeId=' + parentData.key + '&parentNodeName=' + strParentName + '&nodeTypeVal=' + nodeTypeVal + '&parentModuleType=' + parentModuleType;
				  		...
				  		url = '<%=request.getContextPath() %>/cap/ptc/index/AppDetail.jsp?packageId=' + nodeId + '&parentNodeId=' + parentData.key + '&parentNodeName=' + strParentName 
							+ '&nodeTypeVal=' + nodeTypeVal + '&parentModuleType=' + parentModuleType;
					}
				  	break;
				case 3:
				...
				case 4: // 实体
					url = "../entity/EntityList.jsp?packageId=" + node.parent().getData().key +"&packageModuleCode=" + parentData.data.moduleCode;
				  	break;
				case 44: // 新实体
					url = "<%=request.getContextPath() %>/cap/bm/dev/entity/EntityList.jsp?packageId=" + node.parent().getData().key +"&packageModuleCode=" + parentData.data.moduleCode;
				  	break;
				case 5: // 表
					url = "../database/DBObjectMainTab.jsp?packageId=" + node.parent().getData().key;
				  	break;
				case 55: // 新版表
					url = "<%=request.getContextPath() %>/cap/bm/dev/database/DBObjectMainTab.jsp?packageId=" + node.parent().getData().key+"&packagePath="+parentData.data.fullPath+"&packageModuleCode=" + parentData.data.moduleCode;
				  	break;
				case 6: // 服务
					url = "../entity/ServiceObjectMain.jsp?packageId=" + node.parent().getData().key +"&packagePath=" + parentData.data.fullPath;
				  	break;
				case 66: // 新服务
					url = "<%=request.getContextPath() %>/cap/bm/dev/serve/ServiceObjectMain.jsp?packageId=" + node.parent().getData().key +"&packagePath=" + parentData.data.fullPath;
				  	break;
				case 7: // 查询
					cui.alert("链接到查询列表页面");
				  	break;
				case 8: // 界面
					url = "../page/PageList.jsp?packageId=" + node.parent().getData().key;
				  	break;
				case 88: // 新版界面
					url = "<%=request.getContextPath() %>/cap/bm/dev/page/PageHome.jsp?packageId=" + node.parent().getData().key;
				  	break;
				case 9: // 常用数据类型
					url ="../directory/DictionaryList.jsp?classifyId="+ node.parent().getData().key+"&sysModule=Yes&classifyCode="+parentData.data.shortName+"&selectIdent=1+&requestModule=module";
				  	break;
				case 10: // 工作流
					url = "../workflow/WorkFlowMainTab.jsp?packageModuleCode=" + parentData.data.moduleCode;
				  	break;
				case 666: // 工作流的工作台配置
					url = "<%=request.getContextPath() %>/cap/bm/dev/page/designer/WorkflowWorkbenchList.jsp?packageId=" + node.parent().getData().key +"&packageModuleCode="+ parentData.data.moduleCode;
				  	break;
				case 77: // 测试用例
					url = "<%=request.getContextPath() %>/cap/bm/test/design/TestCaseList.jsp?packageId=" + node.parent().getData().key + "&packagePath=" + parentData.data.fullPath;
				  	break;
				default:
		 ...
		 */
		 function doneTreeDataNode (node, childrenNode) {
			 	if(childrenNode) {
			 		childrenNode = addSomeChildNode(childrenNode);
				}
