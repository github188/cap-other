...
var gsNoTopics="没有找到所需标题。";
var gsLoadingDivID="LoadingDiv";
var gsLoadingMsg="正在加载数据，请稍候...";
var gsSearchMsg="搜索...";
	...
	if(gsSK!="")
	{
		...
		if (nNumChar > 1 || gsSK.charCodeAt(0) < 128)
		{
			for (i = 0; i < nNumChar; i++)
			{
				...
				if (chCharCode > 128 && i + 2 <= nNumChar && gsSK.charCodeAt(i + 1) > 128)
				{
					sTwoCharWord = gsSK.substring(i, i + 2);
					sFinalSearchText = sFinalSearchText + sTwoCharWord + " and ";
					bDBCS = true;
				}
				else if (chCharCode <= 128 && i + 2 > nNumChar)
				{
					sFinalSearchText = sFinalSearchText + chChar;
				}
				else if (chCharCode <= 128 && i + 2 <= nNumChar && gsSK.charCodeAt(i + 1) > 128)
				{
					sFinalSearchText = sFinalSearchText + chChar + " ";
				}
				else if (chCharCode <= 128 && i + 2 <= nNumChar && gsSK.charCodeAt(i + 1) < 128)
...
if (window.gbWhUtil&&window.gbWhLang&&window.gbWhProxy&&window.gbWhVer&&window.gbWhMsg)
{
	...
	goErrFont=new whFont("宋体","9pt","#000000","normal","bold","none");
	goHoverFont=new whFont("宋体","9pt","#007f00","normal","normal","underline");
