 ...
 */
@PetiteBean
public class BackMethodTestcaseGenerater extends AbstractMethodTestcaseGenerater {
    ...
    public Map<String, Object> wrapperArgs(EntityVO entity) {
        ...
        String saveRequestJson = this.requestDataJson(facadeName, voName, "queryTodoVOList", queryData);
        args.put("query-list-data", saveRequestJson);
        
        // 根据业务单据进行回退
        Map<String, Object> backData = new HashMap<String, Object>();
        backData.put("workId", "${data['" + idName + "']}");
        backData.put("taskId", "${data['taskId']}");
        backData.put("paramMap", new HashMap<String, Object>());
        String backRequestJson = this.requestDataJson(facadeName, paramName, "back", backData);
        args.put("back-data", backRequestJson);
        return args;
    }
}
